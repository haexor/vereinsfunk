...
function fakeClient(options: {
  existing?: { bucket_id: string; object_path: string; upload_status: string } | null
  ...
  return {
    from: (table: string) => {
      ...
      return {
        select: () => ({
          eq: () => ({
            single: async () => (options.existing === undefined ? { data: { bucket_id: 'raw-media', object_path: 'organizations/x/departments/y/assets/z/foto.jpg', upload_status: 'initiated' }, error: null } : { data: options.existing, error: null }),
          }),
        }),
...
describe('SupabaseUploadService.complete', () => {
  ...
  it('is idempotent: a second call after the asset already left "initiated" does not re-download or re-process', async () => {
    ...
    const client = {
      from: () => ({ select: () => ({ eq: () => ({ single: async () => ({ data: { bucket_id: 'raw-media', object_path: 'x', upload_status: 'ready' }, error: null }) }) }) }),
