...
export function mapImageStylePresetRow(row: Record<string, unknown>) {
  return {
    ...
    frameWidthPx: row.frame_width_px,
    frameCornerRadiusPx: row.frame_corner_radius_px,
    frameBrandAssetId: row.frame_brand_asset_id,
    logoEnabled: row.logo_enabled,
    logoBrandAssetId: row.logo_brand_asset_id,
    logoPosition: row.logo_position,
    logoSizePercent: row.logo_size_percent,
    logoMarginPercent: row.logo_margin_percent,
    filter: row.filter,
...
export function mapPhotoLayoutPresetRow(row: Record<string, unknown>) {
  return {
    ...
    dividerWidthPx: row.divider_width_px,
...
export function firstBlockedBrandField(
  input: BrandOverrideInput,
  lockedFields: readonly string[],
): BrandLockableField | null {
...
export function mapChannelScopeRow(row: Record<string, unknown>, organizationId: string) {
  return {
    id: row.id,
    scope: row.scope,
    scopeId: row.team_id ?? row.department_id ?? organizationId,
...
export function membershipTableFor(
  scope: ScopeLevel,
): 'organization_memberships' | 'department_memberships' | 'team_memberships' {
...
export function mapIntegrationSourceRow(row: Record<string, unknown>) {
  return IntegrationSourceSchema.parse({
    ...
    displayName: row.display_name,
    enabledDomains: row.enabled_domains,
    departmentId: row.department_id,
    endpointUrl: row.endpoint_url,
    fieldMapping: row.field_mapping,
    syncCron: row.sync_cron,
    lossThresholdPercent: row.loss_threshold_percent,
    enabled: row.enabled,
...
export function mapFixtureRow(row: Record<string, unknown>) {
  return FixtureSchema.parse({
    ...
    isHome: row.is_home,
    ownTeamLabel: row.own_team_label,
    ...
    note: row.note,
    announcementDismissedAt: row.announcement_dismissed_at,
    resultDismissedAt: row.result_dismissed_at,
    sourceId: row.source_id,
...
export function mapClubEventRow(row: Record<string, unknown>) {
  return ClubEventSchema.parse({
    ...
    status: row.status,
    invitationDismissedAt: row.invitation_dismissed_at,
    sourceId: row.source_id,
    sourceUpdatedAt: row.source_updated_at,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
