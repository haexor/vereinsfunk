...
describe('Paket 020: Rechtliche Pflichten und Datenschutzbetrieb', () => {
  ...
  const RETENTION_SETTINGS_ROW = {
    ...
    audit_event_days: 1095, consent_evidence_years: 5, updated_at: '2026-08-08T00:00:00Z',
  }
        ...
        forService: () =>
          ({
            from: (table: string) => {
              ...
              if (['audit_events', 'invitations', 'consent_requests', 'publication_media_grants', 'idempotency_keys'].includes(table)) return chain({ data: [], error: null })
        ...
        forService: () =>
          ({
            from: (table: string) => {
              ...
              if (['invitations', 'consent_requests', 'publication_media_grants', 'idempotency_keys'].includes(table)) return chain({ data: [], error: null })
