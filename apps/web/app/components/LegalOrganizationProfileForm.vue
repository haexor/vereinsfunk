...
const legalFormModel = computed({
  get: () => profileDraft.value.legalForm || '__none__',
  set: (v: string) => { profileDraft.value.legalForm = v === '__none__' ? '' : (v as NonNullable<OrganizationProfile['legalForm']>) },
})
const responsiblePersonProfileIdModel = computed({
  get: () => profileDraft.value.responsiblePersonProfileId || '__none__',
  set: (v: string) => { profileDraft.value.responsiblePersonProfileId = v === '__none__' ? '' : v },
})
...
<template>
  <section class="card mb-6 p-6">
    ...
    <div class="grid gap-4 sm:grid-cols-2">
      ...
      <label><span class="mb-1 block text-xs font-semibold">Rechtsform</span><Select v-model="legalFormModel"><SelectTrigger class="rounded-lg p-2 text-sm"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="__none__">Keine Angabe</SelectItem><SelectItem v-for="item in legalForms" :key="item.id" :value="item.id">{{ item.label }}</SelectItem></SelectContent></Select></label>
