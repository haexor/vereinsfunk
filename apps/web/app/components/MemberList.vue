...
defineProps<{
  ...
  roleDraft: Record<string, AssignableRole | ''>
  expiryDraft: Record<string, string>
  roleChangeError: string
  roleChangeSubmitting: string | null
  expirySubmitting: string | null
  trustSubmitAllowedDraft: Record<string, boolean>
  trustRequirementDraft: Record<string, ReviewRequirement>
  trustReasonDraft: Record<string, string>
  trustExpiryDraft: Record<string, string>
        ...
        <div v-for="entry in member.roles.filter((item) => item.membershipId === expandedMembershipId)" :key="`${entry.membershipId}-detail`" :id="`membership-detail-${entry.membershipId}`" class="mt-3 grid gap-3 rounded-xl border border-[#e8e9e2] bg-[#f7f8f4] p-3 sm:grid-cols-2">
          <label v-if="entry.canChangeRole"><span class="mb-1 block text-xs font-semibold">Rolle in {{ scopeName(entry.scope, entry.scopeId) }}</span>
            <div class="flex gap-2">
              <select v-model="roleDraft[entry.membershipId]" class="focus-ring w-full rounded-lg border border-[#dfe0d9] p-2 text-xs"><option v-for="role in availableRolesFor(entry)" :key="role" :value="role">{{ roleLabels[role] ?? role }}</option></select>
              <button type="button" :disabled="roleChangeSubmitting === entry.membershipId || roleDraft[entry.membershipId] === entry.role" class="focus-ring shrink-0 rounded-lg bg-forest px-3 py-2 text-[10px] font-bold text-white disabled:opacity-60" @click="changeRole(entry)">Ändern</button>
            </div>
          </label>
          ...
          <div v-if="canManageTrust(entry)" class="grid gap-3 border-t border-[#e8e9e2] pt-3 sm:col-span-2 sm:grid-cols-2">
            ...
            <label><span class="mb-1 block text-xs font-semibold">Prüfung</span><select v-model="trustRequirementDraft[entry.membershipId]" class="focus-ring w-full rounded-lg border border-[#dfe0d9] p-2 text-xs"><option value="inherit">geerbt</option><option value="always">immer erforderlich</option><option value="waived">befreit (außer Minderjährigenstufe)</option></select></label>
            <label class="sm:col-span-2"><span class="mb-1 block text-xs font-semibold">Begründung</span><input v-model="trustReasonDraft[entry.membershipId]" maxlength="500" class="focus-ring w-full rounded-lg border border-[#dfe0d9] p-2 text-xs" /></label>
            <label><span class="mb-1 block text-xs font-semibold">Befristet bis</span><input v-model="trustExpiryDraft[entry.membershipId]" type="date" class="focus-ring w-full rounded-lg border border-[#dfe0d9] p-2 text-xs" /></label>
