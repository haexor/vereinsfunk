  ...
  <section class="card mb-6 p-6">
    ...
    <ul v-else class="mb-4 space-y-2">
      <li v-for="agreement in agreements" :key="agreement.id" class="rounded-lg bg-[#f7f8f4] px-3 py-2.5">
        ...
        <label class="mt-2 inline-flex items-center gap-2 text-[11px]"><span class="font-semibold">Status ändern:</span><Select :key="agreement.status" :model-value="agreement.status" :disabled="changingAgreementId === agreement.id" @update:model-value="(value: unknown) => changeAgreementStatus(agreement, value as typeof agreement.status)"><SelectTrigger class="rounded-lg p-1.5 text-[11px]"><SelectValue /></SelectTrigger><SelectContent><SelectItem v-for="option in AGREEMENT_STATUS_OPTIONS" :key="option" :value="option">{{ AGREEMENT_STATUS_LABELS[option] }}</SelectItem></SelectContent></Select></label>
    ...
    <form class="grid gap-3 sm:grid-cols-2" @submit.prevent="createAgreement">
      ...
      <label><span class="mb-1 block text-xs font-semibold">Status</span><Select v-model="agreementForm.status"><SelectTrigger class="text-sm"><SelectValue /></SelectTrigger><SelectContent><SelectItem v-for="option in AGREEMENT_STATUS_OPTIONS" :key="option" :value="option">{{ AGREEMENT_STATUS_LABELS[option] }}</SelectItem></SelectContent></Select></label>
