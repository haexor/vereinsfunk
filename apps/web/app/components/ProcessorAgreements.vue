  ...
  <section class="card mb-6 p-6">
    ...
    <ul v-else class="mb-4 space-y-2">
      <li v-for="agreement in agreements" :key="agreement.id" class="rounded-lg bg-[#f7f8f4] px-3 py-2.5">
        ...
        <label class="mt-2 inline-flex items-center gap-2 text-[11px]"><span class="font-semibold">Status ändern:</span><Select :model-value="agreement.status" :disabled="changingAgreementId === agreement.id" @update:model-value="(value: unknown) => changeAgreementStatus(agreement, value as typeof agreement.status)"><SelectTrigger class="w-auto rounded-lg p-1.5 text-[11px]"><SelectValue /></SelectTrigger><SelectContent><SelectItem v-for="option in AGREEMENT_STATUS_OPTIONS" :key="option" :value="option">{{ AGREEMENT_STATUS_LABELS[option] }}</SelectItem></SelectContent></Select></label>
