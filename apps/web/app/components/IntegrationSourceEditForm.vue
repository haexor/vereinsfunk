...
function setRowField(row: MappingRow, value: string) {
  row.field = value === '__none__' ? '' : value
}
</script>
...
<template>
  <div class="mt-4 rounded-xl border border-[#e8e9e2] bg-[#f7f8f4] p-4">
    ...
    <div class="mt-3"><p class="mb-2 text-xs font-semibold">Feldzuordnung</p>
      <div v-for="(row, index) in mappingRows" :key="index" class="mb-2 flex flex-wrap items-center gap-2"><input v-model="row.column" placeholder="Spalte in der Datei" class="focus-ring w-44 rounded-lg border border-[#dfe0d9] p-2 text-xs" /><span class="text-xs text-[#9aa096]">→</span><Select :model-value="row.field || '__none__'" @update:model-value="(value: unknown) => setRowField(row, value as string)"><SelectTrigger class="w-auto rounded-lg p-2 text-xs"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="__none__">Internes Feld wählen …</SelectItem><SelectItem v-for="target in mappingTargetsFor(source.departmentId)" :key="target.value" :value="target.value">{{ target.label }}</SelectItem></SelectContent></Select><button type="button" class="focus-ring text-xs text-[#8a9186] hover:text-amber-800" @click="emit('removeRow', index)">Entfernen</button></div>
