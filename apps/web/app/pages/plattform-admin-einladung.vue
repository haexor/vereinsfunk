...
import { PlatformAdminStatusSchema } from '@vereinsfunk/contracts'
...
async function acceptInvitation() {
  ...
  try {
    ...
    const response = await $fetch<unknown>(`${config.public.apiBase}/v1/platform-admin-invitations/accept`, {
      method: 'POST',
      headers,
      body: { token },
    })
    PlatformAdminStatusSchema.parse(response)
...
<template>
  <div class="text-center">
    ...
    <template v-else-if="status === 'accepted'">
      ...
      <h1 class="font-display text-xl font-extrabold tracking-[-.03em]">Willkommen im Team.</h1>
      <p class="mt-2 text-sm text-[#6c756f]">Die Einladung wurde angenommen.</p>
      <NuxtLink to="/plattform-admin" class="focus-ring mt-5 inline-block rounded-xl bg-forest px-4 py-3 text-sm font-bold text-white">Zur Plattform-Administration</NuxtLink>
    </template>
