...
describe('Zielplattform-Vorgaben', () => {
  ...
  it('lädt Verfügbarkeit und Stilprofile innerhalb der restoringDraft-Klammer', () => {
    ...
    expect(page.replace(/\s+/g, ' ')).toContain(
      "restoringDraft = true await Promise.all([loadProfiles(), loadPlatformAvailability()]) const resumePostId = UuidSchema.safeParse(route.query.postId) const resumeDraftId = UuidSchema.safeParse(route.query.draftId) const resumeSessionId = UuidSchema.safeParse(route.query.sessionId) const resumeMediaAssetId = UuidSchema.safeParse(route.query.mediaAssetId) if (resumeDraftId.success) await loadServerDraft(resumeDraftId.data) else if (resumePostId.success) await loadDraftFromPost(resumePostId.data) else if (resumeSessionId.success) await loadExistingSession(resumeSessionId.data) else if (resumeMediaAssetId.success) { /* PhotoAttachmentList fuellt sich selbst ueber initial-media-asset-ids */ } else { if (route.query.postId !== undefined || route.query.draftId !== undefined || route.query.sessionId !== undefined || route.query.mediaAssetId !== undefined) notice.value = 'Der Link zum Entwurf ist ungültig.' restoreDraft() } restoringDraft = false",
    )
  })
