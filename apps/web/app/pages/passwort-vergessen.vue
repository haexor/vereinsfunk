...
async function submit() {
  ...
  try {
    ...
    // Same message whether or not the email exists -- resetPasswordForEmail itself
    // never reveals that. A thrown error (network/API failure) is a different case
    // and must not be reported as "email is on its way".
    sent.value = true
  } catch {
    errorMessage.value = 'Der Link konnte nicht gesendet werden. Bitte versuche es später erneut.'
  } finally {
    loading.value = false
  }
}
