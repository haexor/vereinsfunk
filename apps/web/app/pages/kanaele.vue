...
<template>
  <div class="mx-auto max-w-[980px] px-5 py-8 sm:px-10">
    ...
    <template v-else>
      ...
      <ChannelCard v-for="channel in channelsState.channels" :key="channel.id" :channel="channel" :members="channelsState.members" :busy="channelsState.busyChannelId === channel.id" :can-manage="channelsState.canManageChannel(channel)" :department-name="channelsState.departmentName" v-model:purpose-draft="channelsState.purposeDraft[channel.id]!" v-model:editorial-imprint-url-draft="channelsState.editorialImprintUrlDraft[channel.id]!" v-model:editorial-privacy-url-draft="channelsState.editorialPrivacyUrlDraft[channel.id]!" v-model:editorial-responsible-profile-id-draft="channelsState.editorialResponsibleProfileIdDraft[channel.id]!" v-model:editorial-responsible-note-draft="channelsState.editorialResponsibleNoteDraft[channel.id]!" :editorial-saving="channelsState.editorialSavingId === channel.id" :editorial-error="channelsState.editorialErrorByChannel[channel.id]" @verify="channelsState.verifyChannel(channel)" @disconnect="channelsState.disconnectChannel(channel)" @save-purpose="channelsState.savePurpose(channel)" @save-editorial="channelsState.saveEditorialFields(channel)" />
