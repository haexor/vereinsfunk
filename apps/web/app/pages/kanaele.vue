...
<template>
  <div class="mx-auto max-w-[980px] px-5 py-8 sm:px-10">
    ...
    <template v-else>
      ...
      <ChannelCard v-for="channel in channelsState.channels" :key="channel.id" :channel="channel" :members="channelsState.members" :busy="channelsState.busyChannelId === channel.id" :can-manage="channelsState.canManageChannel(channel)" :department-name="channelsState.departmentName" :purpose-draft="channelsState.purposeDraft" :editorial-imprint-url-draft="channelsState.editorialImprintUrlDraft" :editorial-privacy-url-draft="channelsState.editorialPrivacyUrlDraft" :editorial-responsible-profile-id-draft="channelsState.editorialResponsibleProfileIdDraft" :editorial-responsible-note-draft="channelsState.editorialResponsibleNoteDraft" :editorial-saving="channelsState.editorialSavingId === channel.id" :editorial-error="channelsState.editorialErrorByChannel[channel.id]" @verify="channelsState.verifyChannel(channel)" @disconnect="channelsState.disconnectChannel(channel)" @save-purpose="channelsState.savePurpose(channel)" @save-editorial="channelsState.saveEditorialFields(channel)" />
