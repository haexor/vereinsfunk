...
alter table public.workflow_outbox
  ...
  add constraint workflow_outbox_status_check check (status in ('pending', 'dispatching', 'dispatched', 'failed', 'cancelled'));
...
returns table (id uuid, workflow_name text, payload jsonb, priority smallint)
...
begin
  ...
  return query with claimable as (
    ...
    update public.workflow_outbox o set status = 'dispatching', claimed_at = now(), attempt = o.attempt + 1, updated_at = now()
    from claimable c where o.id = c.id returning o.id, o.workflow_name, o.payload, o.priority
  ) select claimed.id, claimed.workflow_name, claimed.payload, claimed.priority from claimed;
end;
$$;

create or replace function public.acknowledge_workflow_outbox(p_outbox_id uuid, p_hatchet_run_id text)
returns boolean language plpgsql security definer set search_path = public, pg_temp as $$
declare claimed public.workflow_outbox%rowtype;
begin
  update public.workflow_outbox set status = 'dispatched', dispatched_at = now(), updated_at = now(), last_error_class = null
  where id = p_outbox_id and status = 'dispatching' returning * into claimed;
...
create or replace function public.release_workflow_outbox(p_outbox_id uuid, p_error_class text)
returns boolean language plpgsql security definer set search_path = public, pg_temp as $$
begin
  update public.workflow_outbox set status = 'pending', available_at = now() + interval '5 seconds', claimed_at = null, last_error_class = left(coalesce(p_error_class, 'unknown'), 120), updated_at = now()
  where id = p_outbox_id and status = 'dispatching';
  return found;
end;
$$;

revoke all on function public.claim_workflow_outbox(integer) from public, anon, authenticated;
revoke all on function public.acknowledge_workflow_outbox(uuid, text) from public, anon, authenticated;
revoke all on function public.release_workflow_outbox(uuid, text) from public, anon, authenticated;
grant execute on function public.claim_workflow_outbox(integer), public.acknowledge_workflow_outbox(uuid, text), public.release_workflow_outbox(uuid, text) to service_role;
commit;
