...
-- Review-Nachbesserung zu 2026082004/2026082005: die Position-Obergrenze und die Array-Laenge
-- waren bislang nur ueber die Zod-Grenze am API-Rand durchgesetzt (mediaAssetIds.max(10)) -- ein
-- direkter Service-Role-Aufruf (Skript, kuenftiger Codepfad) konnte am RPC und an der Tabelle
-- vorbei mehr als zehn Anhaenge erzeugen. Beide Grenzen jetzt auch an der eigentlichen
-- Sicherheitsgrenze durchgesetzt, nicht nur an der API-Komfortschicht.
alter table public.composition_session_post_media drop constraint composition_session_post_media_position_check;
alter table public.composition_session_post_media add constraint composition_session_post_media_position_check check (position between 0 and 9);
...
begin
  ...
  -- Tiefenverteidigung: die Zod-Grenze (mediaAssetIds.max(10)) ist keine Sicherheitsgrenze, ein
  -- Service-Role-Aufruf koennte sie umgehen. Dieselbe Obergrenze wie der CHECK auf position.
  if p_media_derivative_ids is not null and coalesce(array_length(p_media_derivative_ids, 1), 0) > 10 then
    raise exception 'too_many_media_derivatives';
  end if;
