...
**Nachtrag (2026-08-12, Review-Runde)**: die Kontingentauslastung in `GET /v1/analytics/summary` rief `count_publications_in_period` je Kontingentzeile einzeln auf — ein Round-Trip pro Zeile (N+1), bei einem Verein mit vielen Abteilungen und Kanälen entsprechend viele Aufrufe pro Auswertungsanfrage. Migration `2026081207_quota_usage_batch.sql` ergänzt `count_publications_for_quotas(organization, reference)`, das dieselbe Frage in einer Abfrage beantwortet. Die Zähllogik bleibt an genau einer Stelle: die neue Funktion ruft `count_publications_in_period` vereinsintern pro Zeile auf, statt die Perioden- und Statusregeln ein zweites Mal auszuschreiben — eine Kopie wäre die nächste Stelle, an der die Auswertung und die Sperre in `schedule_publication()` auseinanderlaufen. Kein Grant an `authenticated` (dieselbe Begründung wie bei der gezählten Funktion: keine eigene Mitgliedschaftsprüfung). Drei neue pgTAP-Assertionen in `supabase/tests/policy_review_routes.test.sql` (Grant-Sperre, eine Zeile je Kontingent, gleiche Zahl wie der Einzelaufruf) und ein API-Test für die Zuordnung Zeile → Auslastung.
