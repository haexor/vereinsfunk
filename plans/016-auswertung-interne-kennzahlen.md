...
- Auswertbare Rohdaten sind **schemamäßig** vorhanden, aber nach Paket 011 überwiegend ohne echte Nutzungsdaten: `posts` mit `status`, `created_at`, `scheduled_for` (`202608020001:151-170`); `submissions` mit `preset_slug`, `communication_goal`, `requested_formats` (`202608030001:4-7`); `post_versions` mit `version_number` (`202608020001:172-192`); `approval_requests` und `approval_decisions` mit `decision` und `reason` (`:200-232`); `publications` mit `status` und `scheduled_for` (`202608030001:92-98`); `workflow_runs` mit `technical_status`, `attempt`, `error_class` (`:81-85`). **Korrektur nach Paket 011**: Paket 011 hat bewusst keine Inhalts-Pipeline vorgezogen — kein Code erzeugt einen `post`/eine `post_version` aus einer `submission`, solange Pakete 001–007 „in Arbeit“ sind (siehe `plans/011-regelwerk-richtlinien-und-kontingente.md`, Abschnitt „Umsetzung: Ergebnis und Abweichungen vom Plan“). Nur `submissions` selbst ist seit 011 echt befüllt (`POST /v1/submissions` persistiert). `posts`, `post_versions`, `approval_requests`, `approval_decisions` und `publications` bleiben in echter Nutzung leer, bis Paket 005/006 den Anlegepfad bauen — dieses Paket sollte entsprechend mit einem ehrlichen Leerzustand starten, nicht mit der Annahme, es gäbe bereits einen Funnel zum Auswerten.
