...
## Entschiedene Fragen (2026-08-09, vor der Umsetzung)

Der Entwurf oben markiert drei Fragen ausdrücklich als offen. Sie sind vor dem Bauen bestätigt worden, nicht still angewendet:

1. **Variante A** (Abschnitt „Umsetzung", Punkt 2): `authz.resolve_review_route` wird als SQL-Nachbau von `resolveReviewRoute`/`buildStageDefinitions` gebaut. `reresolve_approval_route` bleibt für `authenticated` per Grant erreichbar, RLS bleibt zweite Verteidigungslinie wie bei jeder anderen privilegierten Funktion in diesem Projekt (`request_approval`, `decide_approval_stage`, `schedule_publication`, `set_policy_rules`). Preis akzeptiert: Routenlogik existiert zweimal (TS für die Vorschau in der API, SQL für die Durchsetzung) und wird über einen dedizierten Test aneinander gebunden — dieselbe Route für denselben Datenstand, geprüft über beide Wege, bei jeder Änderung an einer der beiden Implementierungen mitzuführen.
2. **`request_approval` wird in diesem Paket mitgehärtet.** Die bereits ausgelieferte Lücke aus Paket 011 (Abschnitt „Risiken", erster Punkt: die Funktion prüft nicht, ob die vom Aufrufer genannten Prüfer den in `review_mode = 'named'` *konfigurierten* entsprechen, auch nicht bei der Minderjährigenstufe) wird geschlossen, indem `request_approval` auf denselben `authz.resolve_review_route`-Aufruf umgestellt wird wie die neue RPC, statt weiterhin `stages` vom Aufrufer zu übernehmen. Das ist eine Verhaltensänderung an produktivem Code aus einem früheren Paket: bislang konnte ein Einreichender mit `post.submit` per direktem RPC-Aufruf einen selbst gewählten, wohlwollenden Vereinskollegen als Prüfer eintragen (inklusive der Minderjährigenstufe), auch wenn die Richtlinie benannte Prüfer verlangt. Nach der Umstellung berechnet `request_approval` die Route ausschließlich selbst; ein vom Aufrufer mitgeschickter `stages`-Parameter entfällt oder wird ignoriert (siehe „Umsetzung", zu bauen: welche der beiden Varianten, Parameter entfernen oder ignorieren, entscheidet die Rückwärtskompatibilitätsprüfung gegen bestehende Aufrufer in `apps/api`).
3. **Eine `rejected`-Stufe bleibt nicht neu auflösbar**, wie im Entwurf angenommen: eine Ablehnung ist eine inhaltliche Aussage über die Version, der reguläre Weg ist eine neue Post-Version. Andernfalls ließe sich eine Ablehnung wegverwalten, statt eine neue Version vorzulegen.
...
- ~~**Offen zu bestätigen**: ob eine `rejected` Stufe wirklich nicht neu aufgelöst werden soll.~~ **Bestätigt (2026-08-09, siehe „Entschiedene Fragen" oben)**: eine `rejected`-Stufe bleibt nicht neu auflösbar. Die Annahme hier ist, dass eine Ablehnung eine inhaltliche Aussage über die Version ist und eine neue Version erzeugen soll — nicht eine Wiederholung derselben Version mit anderen Prüfern. Wer das anders sieht, öffnet damit den Weg „Ablehnung wegverwalten“, und das wäre gegen den Zweck der Freigabe.

## Umsetzung: Ergebnis und Abweichungen vom Plan

Umgesetzt wie in „Entschiedene Fragen" festgelegt: Variante A (SQL-Nachbau der Routenauflösung, `authz.resolve_review_route`), `request_approval` auf denselben Mechanismus umgestellt (Signaturänderung: kein `stages`-Parameter mehr, alte Zwei-Parameter-Funktion per `drop function` entfernt statt nur ersetzt), `rejected`-Stufen bleiben von der Neuauflösung ausgeschlossen. Migration `2026081002_review_route_reresolution.sql`: `authz.resolve_review_route`, `authz.assert_valid_stage_list` (aus `request_approval` herausgelöst), `public.reresolve_approval_route`, Tabelle `approval_route_changes`, `invalidated_at`-Durchsetzung in `authz.can_decide_stage`/`decide_approval_stage`. API: `POST /v1/approval-requests/:id/reresolve`, `GET /v1/approval-requests/stalled`, `approval_route_changes` in `GET /v1/post-versions/:id/approval`. Oberfläche: neuer Abschnitt „Festhängende Freigaben deiner Ebene" auf `pages/freigaben.vue` mit Neuauflösen-Aktion (Begründungsfeld, Bestätigung). `pnpm lint/typecheck/test/build` grün (257 API-Tests, davon 11 neu; 58 Contracts-Tests; 534 pgTAP-Assertionen, davon 34 neu in `supabase/tests/review_route_reresolution.test.sql`, plus 2 auf die neue Rechtslage umgestellt und 2 zusätzliche in `policy_review_routes.test.sql`), `db:reset`/`db:test` grün. (Nach dem CodeRabbit-Review zu PR #31 um einen Regressionstest je Ebene ergänzt: `ambiguous_stage_mapping`-Fehlerabbildung in der API, doppelte Stufenzuordnung in der Migration.)

**Sieben Funde beim Bauen, alle durch tatsächliche Ausführung gegen echtes Postgres/Vitest gefunden (kein reines Lesen des Diffs) und behoben, mit Regressionstest:**

1. **Vorbestehender, seit Paket 011 nie ausgeführter Bug in `decide_approval_stage`**, projektweit relevant: `update posts set status = case when target_decision = 'changes_requested' then 'changes_requested' else 'cancelled' end` — ein `CASE` mit ausschließlich Text-Literalen löst ohne expliziten Cast auf `text` auf, nicht auf den Zielspaltentyp. Die Zuweisung an `posts.status` (Enum `post_status`) scheiterte deshalb bei **jeder** Ablehnung oder Änderungsanfrage seit Paket 011 mit einem SQL-Typfehler (500) — unbemerkt, weil kein bisheriger Test `decide_approval_stage` mit `rejected`/`changes_requested` gegen echtes Postgres ausgeführt hat (nur TS-seitige Tests mit gemocktem Client). Behoben durch expliziten Cast (`::public.post_status`), da diese Funktion ohnehin für den `invalidated_at`-Guard neu gefasst wurde. Regel für künftige Pakete: ein `CASE` mit reinen String-Literalen, das an eine Enum-Spalte zugewiesen wird, braucht in einem `UPDATE ... SET` (anders als in einem `INSERT ... VALUES`, das offenbar den Zielspaltentyp in die Zweige durchreicht) einen expliziten Cast.
2. **`scope`-Mehrdeutigkeit in `authz.resolve_review_route`**: `RETURNS TABLE(..., scope, ...)` legt automatisch eine gleichnamige PL/pgSQL-Variable an, die `policy_settings.scope`/`member_review_trust.scope` in jeder unqualifizierten Abfrage im Funktionskörper verdeckt. Behoben durch durchgängige Tabellenqualifizierung. Gilt für jede künftige `RETURNS TABLE`-Funktion mit einer Spalte, deren Name mit einer im Funktionskörper gelesenen Tabellenspalte kollidiert.
3. **Enum-Vergleich ohne Cast**: `membership.role = reviewer.role` scheiterte mit „operator does not exist“, weil `organization_memberships.role`/`department_memberships.role`/`team_memberships.role` Enum-Typen sind, `policy_reviewers.role` aber `text`. Behoben durch `::public.organization_role`/`::public.department_role`/`::public.team_role` an den drei betroffenen Stellen.
4. **Fehlende RLS-Sichtbarkeit für Verwaltende, projektweit relevant**: `approval_requests_select`/`approval_stages_select`/`approval_route_changes_select` deckten vor diesem Paket nur Organisationsrolle, zugewiesene Prüfer und den Autor ab — eine Person mit `department.manage`, die selbst weder Prüfer noch Autor ist, konnte eine Anfrage ihrer eigenen Abteilung nicht einmal per `SELECT` finden, um ihre `approval_request_id` für die Neuauflösung zu nennen (obwohl `reresolve_approval_route` als `SECURITY DEFINER` die Berechtigung selbst prüft und nicht auf RLS angewiesen ist). Alle drei Policies um eine `department.manage`-Klausel erweitert. Betrifft nur Sichtbarkeit, keine Schreibrechte — die eigentliche Durchsetzung bleibt vollständig in den RPCs.
5. **Fensterfunktion in einer Aggregatfunktion**: `jsonb_agg(elem || jsonb_build_object('position', row_number() over ()))` ist in Postgres ungültig (Fensterfunktionen dürfen nicht direkt in Aggregatfunktionsargumenten stehen). Behoben durch eine vorgeschaltete Unterabfrage, die `row_number()` als eigene Spalte berechnet, bevor aggregiert wird.
6. **Race-sichere Positionsverschiebung**: `approval_stages` trug `unique (approval_request_id, position)` ohne `deferrable` — eine Neuauflösung, die mehrere Positionen derselben Anfrage gleichzeitig verschiebt, hätte schon bei der ersten Einzel-Aktualisierung mit einem noch belegten Zielwert kollidieren können, obwohl der Endzustand gültig ist. Der Constraint wurde auf `deferrable initially immediate` umgestellt (ALTER CONSTRAINT unterstützt das für UNIQUE-Constraints nicht, deshalb drop+neu anlegen); `reresolve_approval_route` setzt ihn zu Beginn testweise auf `deferred`.
7. **Zwei pgTAP-Tests aus Paket 011 riefen `request_approval` mit dem alten `(uuid, jsonb)`-Aufruf auf** und die Signaturänderung hätte sie brechen lassen — auf `authz.assert_valid_stage_list` umgestellt (dieselbe Prüfung lebt jetzt dort), plus zwei neue Tests, dass weder `authz.assert_valid_stage_list` noch `authz.resolve_review_route` direkt von `authenticated` aufrufbar sind.

**Implementierungsentscheidung, vom Plan nicht bis ins Detail vorgegeben**: bei der Neuauflösung wird jede nicht-abgeschlossene Stufe (auch eine mit bereits erweitertem `reviewer_snapshot`) zunächst auf `pending` zurückgesetzt; anschließend öffnet der letzte Schritt genau die niedrigste davon. Das hält „genau eine Stufe öffnen" (Schritt 9) wörtlich ein, statt den bisherigen `open`/`stalled`-Status einer erweiterten Stufe unangetastet zu lassen, was bei einer vorgezogenen neuen Stufe zwei gleichzeitig offene Stufen erzeugen würde. Bereits abgegebene `approval_decisions` bleiben davon unberührt.

**Bewusst kleiner als der ursprüngliche Entwurf**: `GET /v1/approval-requests/stalled` deckt nur zwei der drei im Plan („Oberfläche") genannten Auslöser ab — überfällig (`isOverdue`) und invalidiert (`invalidated_at`). Der dritte, „der `reviewer_snapshot` ist nicht mehr erfüllbar" (`unresolvableReviewers`: keine der genannten Personen ist noch aktives Mitglied, oder weniger nicht-abgelaufene Mitglieder als `minimumApprovals`), ist nicht gebaut — er bräuchte einen Mitgliedschafts-Ablaufabgleich je Snapshot-Eintrag über alle offenen Stufen eines Vereins, ein eigener, klar abgegrenzter Nachtrag. Ebenfalls nicht gebaut: eine eigene Detailseite, die `approval_route_changes` aus `GET /v1/post-versions/:id/approval` tatsächlich anzeigt — dieser Endpunkt hat (vorbestehend, siehe `plans/README.md`, „Bewusst offen gelassene Punkte") noch keinen UI-Konsumenten, weil `erstellen.vue` weiterhin keinen Trigger für `request_approval` hat; der Verlauf ist über die API bereits abrufbar und getestet, wartet aber auf dieselbe UI-Anbindung wie der Rest der Freigabeansicht. Benachrichtigung neu benannter Prüfer bleibt wie im Plan vorgesehen draußen (Paket 004).
