...
Neue SQL-Funktion `claim_stalled_generation_candidates(p_limit integer default 20)`, analog `claim_workflow_outbox` (`supabase/migrations/2026081101_workflow_outbox_dispatch.sql:22-30`): claimt bis zu `p_limit` Zeilen aus `generation_candidates` mit `status = 'generating' and updated_at < now() - interval '15 minutes'` per `for update skip locked` und gibt `id, composition_session_id, organization_id, generation_intent, revision_instruction` zurück. Diese eine `UPDATE ... RETURNING`-Anweisung ist bereits gegen mehrere gleichzeitig laufende Worker-Replikas sicher (kein zusätzlicher Aufruf von `acquire_generation_candidate` nötig).

**Review-Fix (PR #52):** Ursprünglich setzte `claim_stalled_generation_candidates` die Zeile in derselben Anweisung atomar auf `status = 'failed'` — das beendete den Kandidaten aber, bevor der Worker über das Netzwerk einen Ersatzversuch erzeugt hatte. Ein Absturz oder ein unerwarteter Fehler zwischen Claim und Ersatzversuch hätte den Kandidaten dann dauerhaft ohne jede Wiederherstellung verloren (exakt die Fehlerklasse, die dieser Plan eigentlich schließen soll). Stattdessen erneuert der Claim nur `generation_lease_token`/`updated_at` (analog zur Reeroberung in `acquire_generation_candidate`, nicht zu `mark_generation_candidate_failed`) und gibt zusätzlich `generation_lease_token` zurück; die Zeile bleibt `generating` und wird claimbar, falls sie nie finalisiert wird. Eine neue, ebenfalls fenced-e Funktion `finalize_stalled_generation_recovery(p_candidate_id, p_session_id, p_lease_token, p_failure_code)` setzt den alten Kandidaten erst `failed`, nachdem der Worker den Ersatzversuch erzeugt hat (oder festgestellt hat, dass keiner möglich ist) — die begleitende `composition_sessions`-Aktualisierung ist dabei absichtlich `best effort` (kein `if not found then raise`), weil ein bereits erfolgreich erzeugter Ersatzversuch die Sitzung längst weiterbewegt hat.

Neue Datei `apps/worker/src/generationRecovery.ts`: `scanAndRecoverStaleCandidates()` ruft `claim_stalled_generation_candidates` wiederholt auf (bis eine leere Seite zurückkommt oder eine Sicherheitsobergrenze an Iterationen erreicht ist). Für jede zurückgegebene Zeile: die zugehörige `composition_sessions`-Zeile laden (`organization_id, department_id, team_id, preset_slug, communication_goal, requested_formats, source_material, style_profile_id, style_profile_snapshot, effective_config_snapshot, source_revision, input_hash, created_by` — dieselben Felder, die `POST /v1/text-workshop/sessions/:id/generations` bereits liest, `apps/api/src/app.ts:1507-1511`), dann `create_text_generation_session` aufrufen mit denselben Werten plus `p_generation_intent`/`p_revision_instruction` der stallenden Kandidatenzeile (nicht `'revise'` fest verdrahtet — ein hängender `'initial'`-Versuch wird als `'initial'`-Versuch wiederholt), `p_candidate_input_hash = sha256(staleCandidateId || ':recovery')` (deterministisch, garantiert verschieden vom Hash des beendeten Versuchs, damit `create_text_generation_session`s Dedup-Zweig nicht die soeben beendete Zeile zurückgibt und ein erneuter Versuch nach einem Absturz denselben Ersatzversuch wiederfindet statt einen zweiten anzulegen), `p_created_by` aus der Sitzung (Provenienz zeigt weiterhin auf ein echtes Mitglied), `p_triggered_by = 'automatic_recovery'`, frische `p_correlation_id`/`p_idempotency_key`. Danach — ob erfolgreich, limitiert, oder weil keine Sitzung mehr gefunden wurde — `finalize_stalled_generation_recovery` aufrufen, um den alten Kandidaten `failed` zu setzen. Wirft der `create_text_generation_session`-Aufruf `composition_session_candidate_limit_reached` (Step 3), wird das geloggt, aber trotzdem finalisiert — die Sitzung endet ehrlich `failed`, sichtbar für ein Mitglied über die bestehende Fehlermeldung. Wirft irgendein anderer, unerwarteter Fehler, bleibt die Finalisierung aus und die Zeile ist nach weiteren 15 Minuten erneut claimbar.
...
- [x] `claim_stalled_generation_candidates`/`finalize_stalled_generation_recovery` existieren, sind gegen gleichzeitige Aufrufer sicher (pgTAP) und verlieren einen Kandidaten nicht dauerhaft, wenn der Ersatzversuch zwischen Claim und Finalisierung abstürzt (siehe Review-Fix in Step 4).
