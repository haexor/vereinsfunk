...
>
> **Status-Nachtrag (2026-08-11, Gate-Durchsetzung)**: `evaluateMediaGate` ist jetzt als echter Blocker verdrahtet — aber bewusst nicht an der ursprünglich vermuteten Stelle (`decide_approval_stage`) und bewusst nicht vollständig. `schedule_publication` (`2026081107_media_gate_publish_enforcement.sql`) ist laut eigenem Code-Kommentar die tatsächliche, per Grant direkt erreichbare Durchsetzungsgrenze (dasselbe Muster wie die Kanal-/Kontingent-Regeln aus Plan 012); dort blockiert jetzt ein konservativer SQL-Kern hart: `scan_pending`, `derivative_stale`, `face_pending`, `consent_invalid` (widerrufen/abgelöst/außerhalb Gültigkeit/Minderjährige ohne Guardian-Unterschrift). `POST /v1/publications/:id/execute` lädt denselben konservativen Filter (`HARD_PUBLISH_BLOCKERS`, `apps/api/src/routes/policies.ts`) über die vorhandene `computeMediaGateBlockersForPostVersion` erneut, bevor `publisher.publish()` läuft (Pflichtszenario 5: Widerruf nach Freigabe). Ein Beitrag ohne jedes Medium (Text-only-Pilot, Plan 033) hat keine `post_media`-Zeilen und bleibt gate-frei.
>
> Bewusst **nicht** hart blockiert: `minor_review_required` (`minorReviewConfirmed` ist in `computeMediaGateBlockersForPostVersion` dauerhaft `false` — ein harter Block wäre für jeden Beitrag mit einer minderjährigen Person permanent unerfüllbar; der eigentliche Minderjährigenschutz läuft über die `is_minor_stage`-Freigabestufe aus Paket 011/024) sowie `consent_scope_mismatch`, `naming_not_allowed`, `sensitive_text_data` (hängen an `evaluateConsent`/`scanTextForSensitiveData`, rund 150 Zeilen Unicode-Regex- und Scope-Abgleichslogik — eine SQL-Portierung müsste dieselbe Rechtslogik dauerhaft in zwei Sprachen synchron halten). Diese bleiben informativ über `GET /v1/approval-stages/mine`, dokumentierte Restlücke. Ebenfalls nicht im SQL-Kern: `person_left` — es hängt an der über Verein → Abteilung → Team vererbten Richtlinie `consentExpiresOnLeave`, deren Standardwert in `packages/domain` liegt; der Fall wird von der TS-Prüfung in `POST /v1/publications/:id/execute` abgedeckt (Einplanen gelingt, Veröffentlichen nicht).
>
> **Code-Review-Nachtrag (2026-08-11)**: Drei Funde behoben. (1) Die Migration trug den bereits von `2026081105_text_generation_review_fixes.sql` belegten Versionspräfix — `supabase_migrations.schema_migrations.version` ist der Primärschlüssel und wird allein aus dem Ziffernpräfix abgeleitet, jedes `db reset`/`db push` nach dem Merge wäre daran gescheitert; umbenannt auf `2026081107`. (2) `POST /v1/post-versions/:id/schedule` hatte keine Zuordnung für den neuen `media_gate_blocked`-Fehler — die fachlich korrekte Absage kam als 500 ohne Angabe des blockierenden Mediums beim Aufrufer an; jetzt 409 mit `blockers`-Liste, gleiche Antwortform wie der Execute-Endpunkt. (3) Die erneute Prüfung in `POST /v1/publications/:id/execute` lief über den Nutzer-Client: `directory_people` ist per RLS an `directory.read` gebunden, und genau die veröffentlichende Organisationsrolle `social_manager` hat dieses Recht nicht — die Verzeichnisperson kam leer zurück, `subjectIsMinor`/`personLeft` fielen auf `false` und die Prüfung fiel still offen, ausgerechnet für die beiden Fälle, die zwischen Freigabe und Ausführung neu entstehen können. Die Gate-Lesungen laufen jetzt über den Service-Client (der Aufrufer ist durch `requirePermission` bereits autorisiert, nach außen geht nur die Blocker-Liste), abgesichert durch einen Regressionstest, dessen `forUser`-Fake für `directory_people` wirft.
>
> Dieselbe Lücke bestand vorbestehend auch in der informativen Anzeige `GET /v1/approval-stages/mine` und ist mit behoben: dort ist sie noch breiter, weil `post_media`/`media_derivatives`/`consent_records` per RLS `is_organization_member` verlangen — eine reine Abteilungs-`approver`-Rolle sah gar keine Medienzeile und damit die leere Blockerliste, die wie „nichts zu beanstanden" aussieht. Welche Stufen jemand sieht, bleibt weiterhin seine eigene Sichtbarkeit über den Nutzer-Client; nur die Blockerauflösung einer Stufe, für die er ohnehin als Prüfer eingetragen ist, läuft über den Service-Client. Die Anforderung ist jetzt am Helfer `computeMediaGateBlockersForPostVersion` selbst dokumentiert, damit ein künftiger Aufrufer sie nicht wieder unterläuft.
>
> Weiterhin unverändert offen (nicht Teil dieser Änderung): der Upload-Pfad selbst (`LocalUploadService` bleibt ein Stub; `erstellen.vue` hat seit Plan 033 nicht einmal mehr einen Datei-Input), Worker-seitige EXIF-Normalisierung/Virenscan, die Face-Region-Review-UI, `assertApprovalSnapshot` (weiterhin ohne Aufrufer) und das Schreiben von `approval_media_snapshots` (weiterhin nie befüllt).
