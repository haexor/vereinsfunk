...
| Docker-Image lokal bauen und Boot-Hook prüfen | `docker build -f apps/api/Dockerfile -t vereinsfunk-api:test .` dann `docker run --rm -e DATABASE_URL=... -e ... vereinsfunk-api:test` | Log zeigt Migrationslauf vor „server listening"; Container beendet sich bei absichtlich kaputter Test-Migration mit Exit ≠ 0 |
...
- `apps/api/src/server.ts`: Aufruf **vor** `buildApp()`/`app.listen()`, vor dem bestehenden `bootstrap_platform_admin`-Aufruf.
- `apps/worker/src/index.ts`: identischer Aufruf ganz am Anfang, vor jeder Hatchet-Worker-Registrierung.
...
`packages/config/src/index.ts`: `DATABASE_URL: z.string().min(1)` (oder eine striktere Postgres-URL-Validierung, falls das bestehende `optionalUrl`/`z.url()`-Muster das ohne Weiteres hergibt — `postgresql://`-Schemas prüfen, `z.url()` könnte das Protokoll ablehnen) zu `ApiEnvironmentBaseSchema` und zum Worker-Pendant hinzufügen, in die `required`-Liste (Zeile ~83) aufnehmen. Lokale `.env`/`supabase/seed`-Dokumentation ergänzen, welchen Wert die lokale Supabase-Instanz dafür liefert (`supabase status` zeigt die lokale Connection-URL).

**Verify**: `pnpm --filter @vereinsfunk/config test` — neuer Testfall, dass ein fehlendes `DATABASE_URL` den Server- bzw. Worker-Start mit einer klaren Fehlermeldung verweigert, analog zu den bestehenden Tests für `SUPABASE_URL`.
...
**Verify**: `packages/db-migrate` bekommt einen eigenen Test mit einem Fake-Kindprozess (kein echter Datenbankzugriff im Unit-Test) — Exit 0 löst nichts aus, Exit ≠ 0 wirft mit dem `stderr`-Inhalt in der Fehlermeldung.

### Step 3: Boot-Hooks in `apps/api` und `apps/worker`, Laufzeit-Images erweitern

`apps/api/src/server.ts`: `await runPendingMigrations(...)` als **allererste** Zeile, noch vor `parseApiEnvironment()`s Folgeaufrufen und vor `buildApp()` — nichts im Prozess darf vor einer erfolgreichen Migration auch nur eine Route registrieren oder eine DB-Abfrage vorbereiten. Der bestehende `bootstrap_platform_admin`-Aufruf (Zeile 10-15) bleibt unverändert danach.
...
**Verify**: das Kommando aus der Tabelle oben („Docker-Image lokal bauen und Boot-Hook prüfen") — ein frisch gebautes Image gegen die lokale Supabase-Instanz zeigt den Migrationslauf im Log vor „server listening"; ein Testlauf mit einer absichtlich fehlerhaften zusätzlichen Migrationsdatei beendet den Container mit Exit ≠ 0 und ohne dass die Anwendung je auf Port `4201` lauscht.
...
- [ ] `packages/db-migrate` existiert, ist getestet, und wird von `apps/api/src/server.ts` sowie `apps/worker/src/index.ts` als allererster Schritt aufgerufen.
- [ ] Beide Laufzeit-Images enthalten `supabase/migrations/` und die `supabase`-CLI; ein lokal gebautes Image wendet beim Start tatsächlich ausstehende Migrationen an und startet den Server/Worker erst danach.
- [ ] Eine absichtlich kaputte Test-Migration lässt den Container mit Exit ≠ 0 fehlschlagen, ohne dass der Server je auf seinem Port lauscht.
