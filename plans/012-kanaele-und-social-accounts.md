...
- **Kein Hatchet-Cron für Token-Ablaufprüfung und Schlüsselrotation**: Paket 004 ist als technischer Outbox-/Worker-Unterbau abgeschlossen, eine fachliche Scheduler-Registrierung für diese Aufgaben fehlt weiterhin — dieselbe Lage wie bei `mark_stalled_approval_stages()` aus Paket 011. `flag_channels_needing_reconnect()` und `cleanup_expired_oauth_state()` existieren als fertige, nur für `service_role` aufrufbare SQL-Funktionen, die ein künftiger Scheduler lediglich verdrahten muss.
...
### Zweite Review-Runde (PR-Review und CodeRabbit)

- **Leere `allowedChannelIds`-Liste war in `schedule_publication` wirkungslos**: die Prüfung lief nur bei `jsonb_array_length(...) > 0` und ließ damit bei einer leeren Liste jeden Kanal zu — genau die Verwechslung von `null` („keine Einschränkung“) und `[]` („nichts erlaubt“), die Plan 011 als häufigste Fehlerquelle benennt und die `resolveAvailableChannels` in `packages/domain` korrekt umsetzt. Da die RPC per Grant direkt erreichbar ist, war die Richtlinie „kein Kanal erlaubt“ vollständig umgehbar. Behoben, mit pgTAP-Regressionstest (Test 31), der ohne den Fix fehlschlägt.
- **`GET /v1/channels/connect/:platform/start` wies jeden Vereinskanal-Start aus der Oberfläche ab**: die Seite übergibt `ownerDepartmentId` auch beim Vereinskanal (als `null`), `$fetch` hängt einen `null`-Wert als schlüssellosen Query-Parameter an, und Fastify liest ihn als leeren String — die `UuidSchema.nullish()`-Prüfung schlug damit fehl (400). Die Route normalisiert den leeren String jetzt und validiert die Bedingung zwischen `ownerScope` und `ownerDepartmentId` über `ChannelConnectStartRequestSchema` aus `packages/contracts` statt inline. Zwei API-Tests decken beide Richtungen ab.
- **`responsible_profile_id` war nicht an die Mitgliedschaft gebunden**: die Spalte hat nur einen Fremdschlüssel auf `profiles`. Ohne Prüfung ließ sich ein Mitglied eines fremden Vereins als verantwortliche Person eintragen und `require_channel_responsible` damit mit einer vereinsfremden Person erfüllen. `PATCH /v1/channels/:id` antwortet jetzt mit `422 responsible_not_a_member`.
- **Geheimnisse standen in Meta-URLs**: `exchangeCode`/`exchangeForLongLivedToken` übergaben `client_secret`, `listAvailableAccounts`/`verifyToken` das Zugriffstoken als Query-Parameter — URLs landen in Proxy-, Server- und Fehlerlogs. Token reisen jetzt im `Authorization: Bearer`-Header, `appSecret` im POST-Body. Alle vier Aufrufe haben zusätzlich ein `AbortSignal`-Timeout (10 s), sonst hing der OAuth-Callback im Anfrage-Thread bis zum Socket-Timeout.
- **Keine `audit_events` auf den neuen Schreibrouten**: alle Bestandsrouten auditieren administrative Änderungen, die Kanalrouten taten es nicht — wer einen Kanal getrennt, geprüft oder für eine Abteilung freigegeben hat, war nicht nachweisbar. Ergänzt für Verbinden, Ändern, Prüfen, Trennen sowie Freigabe erteilen/entziehen, dazu die beiden Prüferrouten aus Paket 011, die dieselbe Lücke hatten. Der wiederkehrende Insert liegt jetzt in einem lokalen Helfer `recordAuditEvent`.
- **Fehler beim Aufräumen des Pending-Datensatzes wurde verworfen**: schlug das `delete` auf `oauth_pending_connections` fehl, blieben die versiegelten Seiten-Tokens der abgelehnten Konten bis `expires_at` liegen, und die Antwort meldete trotzdem Erfolg. Wird jetzt geloggt (nicht geworfen — der Kanal existiert zu dem Zeitpunkt bereits).
- **`savePurpose` löschte den gespeicherten Zweck beim Verlassen des Feldes**: `purposeDraft` war nie vorbelegt, der gespeicherte Wert stand nur im Platzhalter — ein Hineinklicken und Verlassen schickte `purpose: null`. Der Entwurf wird jetzt aus `channel.purpose` initialisiert, der Platzhalter ist neutral.
- **Zuordnungsmatrix aktivierte Checkboxen, die der Server ablehnt**: der `disabled`-Ausdruck ließ auch `social_account.manage` auf der Ziel-Abteilung genügen, während `channel_scopes_insert` den Kanal**besitz** prüft. Auf `canManageChannel(channel)` reduziert, `canManageDepartmentChannels` damit entfallen.
- **Nicht übernommen**: CodeRabbits Hinweis auf eine mehrdeutige Vereinszeile in `policy_settings` — `policy_settings_org_unique` (partieller Unique-Index auf `organization_id where scope = 'organization'`) zusammen mit dem CHECK-Constraint aus `2026080604` garantiert bereits genau eine Zeile je Verein. Der Testteil des Hinweises (ein redundanter zweiter `insert` mit `on conflict do nothing`) war berechtigt und wurde entfernt.
