...
> **Umsetzung (2026-08-11):** Erledigt für die technische Orchestrierungsgrenze: lokaler
> self-hosted Hatchet-Nachweis, echte SDK-Workerregistrierung, strikte ID-only Verträge,
> transaktionales Outbox-/Run-Mapping, Lease-CAS, Retry-/Fehlerklassen und RLS-/pgTAP-Tests
...
>
> **Historischer Abgleich mit Paket 032 (2026-08-10)**: Die Datenbasis für Kompositionssitzungen und Generierungskandidaten legte damals absichtlich keine Outbox-Zeile an und rief keinen Provider auf. Seit Plan 033 ist `generate-text-post` mit einem Kandidaten-spezifischen ID-only-Payload registriert; ein separater `revise-text-post`-Workflow bleibt bewusst unregistriert, weil Revisionen denselben getesteten Executor nutzen.
