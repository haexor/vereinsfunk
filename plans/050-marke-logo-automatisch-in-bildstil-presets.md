...
- Aufräumen bestehender, jetzt verwaister `image_style_presets`-Datensätze mit gesetztem `logo_brand_asset_id` — die Migration löscht die Spalte ersatzlos. Für `logo_primary`/`logo_light` ist das nach Design-Entscheidung 1 unkritisch (identische Ausgabe wie das künftig aufgelöste Marke-Logo). Ob das auch für die übrigen `LOGO_ASSET_KINDS`-Varianten gilt, ist unbelegt — siehe Bestandsprüfung im Datenmodell-Abschnitt.
...
**Bestandsprüfung vor dem Drop**: `select logo_brand_asset_kind, count(*) from public.image_style_presets where logo_brand_asset_id is not null group by logo_brand_asset_kind;` gegen die Produktionsdatenbank ausführen. Kommt nur `logo_primary`/`logo_light` vor, ist der Wechsel auf das automatisch aufgelöste Marke-Logo laut Design-Entscheidung 1 deckungsgleich. Taucht `logo_dark`, `logo_mark`, `wordmark` oder `watermark` auf, betrifft mindestens ein bestehendes Preset eine Variante, die die neue Auflösung (immer `logo_primary`, siehe unten) nicht liefert — anhalten und mit dem Nutzer klären, ob der Rendering-Wechsel für diese Presets akzeptabel ist, bevor die Migration geschrieben wird.
...
4. Neue Funktion `resolveEffectiveBrandLogoAssetId` (Geschwister zu `loadResolvedBrandColors`, gleiche Datei oder `packages/domain/src/brand.ts` falls dort passender): löst pro Ebene (Verein/Abteilung/Mannschaft) analog zu `resolveBrand` auf, aber mit einer zusätzlichen Vor-Abfrage für die Vereinsebene, da `organization_brand_profiles` kein `logo_asset_id`-Feld führt — `select id from brand_assets where organization_id = :orgId and department_id is null and team_id is null and kind = 'logo_primary' and status = 'ready' order by created_at desc limit 1` liefert den Wert, den `resolveBrand` als `organization.logoAssetId` bekommt. `order by created_at desc limit 1` ist Verteidigung in der Tiefe: der bestehende Upload-Pfad (`POST /v1/organizations/:id/brand/logo`, `brand.ts:150-154`) setzt die Vorgängerzeile bereits vor dem Insert auf `status='replaced'`, wodurch im Normalfall ohnehin nur eine `ready`-Zeile je Kind existiert — ohne explizite Sortierung würde ein `.single()` bei einer seltenen Race zwischen zwei gleichzeitigen Uploads aber mit einem Fehler abbrechen statt deterministisch die neueste Zeile zu liefern. Department-/Team-Ebene liefern ihr `logo_asset_id` bereits über die bestehenden Mapper (`apiMappers.ts:88-104`). `loadResolvedBrandColors` wird um dieses Feld erweitert (Rückgabetyp `{ primaryColor, accentColor, logoAssetId: string | null }`) — ein Aufrufer, der es ignoriert, bleibt unverändert lauffähig.
5. `renderImageStyle`/die Route (~336-350): `input.logoAssetBuffer` wird über die aufgelöste `logoAssetId` geladen (`downloadBrandAssetBuffer`), nicht mehr über `preset.logoBrandAssetId`. Ist `logoEnabled=true`, aber die aufgelöste `logoAssetId` `null` (kein Logo auf keiner Ebene hinterlegt), gibt die Route einen expliziten Fehler `reply.code(409).send({ error: 'brand_logo_missing', correlationId: request.id })` statt eines rohen 500 aus `applyLogoWatermark`s `throw new Error('missing logo asset buffer')` — derselbe String-Code-Mechanismus wie `platform_not_available`/`media_asset_not_reviewed` in `content.ts`, kein neuer Fehlervertrag nötig. `erstellen.vue` fängt beim Style-Render-Aufruf (siehe PR 1) `code === 'brand_logo_missing'` genauso ab wie die bestehenden Fälle in `startTextGeneration`s catch-Block (`erstellen.vue:262-279`) und zeigt eine spezifische Meldung („Für diesen Bereich ist kein Vereinslogo hinterlegt.") statt der generischen Fehlermeldung.
   - *Prüfung*: Vitest — Preset mit `logoEnabled=true` und Marke ohne Logo liefert `409 brand_logo_missing`; Verein mit Logo, Abteilung ohne eigenes Logo erbt das Vereinslogo; Abteilung mit eigenem `department_brand_profiles.logo_asset_id` überschreibt es; Pixelprobe bestätigt, dass das im Ergebnis komposittete Logo tatsächlich das aufgelöste Asset ist (nicht mehr `preset.logoBrandAssetId`); mehrere historische `brand_assets`-Zeilen mit `kind='logo_primary'` und `status='ready'` (z. B. durch direktes Test-Fixture-Setup ohne den Supersede-Schritt) liefern deterministisch die zuletzt erstellte.
...
1. `accept_text_generation_candidate` (neue Migration, PR 0 dieses Pakets oder eigene Folgemigration): Rückgabe-`jsonb` um `postMediaIds` (Array, nach `position` sortiert) ergänzen — **an beiden Return-Stellen**, nicht nur beim regulären Insert-Pfad. Der bestehende Idempotenz-Kurzschluss (Migration `2026082005`: `if candidate.status = 'accepted' then return jsonb_build_object('postVersionId', ..., 'alreadyAccepted', true); end if;`) hat aktuell keinen Zugriff auf die zugehörigen `post_media`-Zeilen und muss dafür `select array_agg(id order by position) from public.post_media where post_version_id = candidate.accepted_post_version_id` ergänzen. Ohne das liefert ein wiederholter Accept-Aufruf (z. B. nach einem fehlgeschlagenen `request-approval`, siehe `erstellen.vue:294-296`) keine `postMediaIds`, wodurch der Style-Render-Schritt in PR-1-Schritt-3 bei einem Retry trotz gewähltem Preset stillschweigend übersprungen würde. `apps/web`s Response-Schema (`erstellen.vue:287-290`, `z.union([...])`) bekommt `postMediaIds: z.array(UuidSchema)` in **beiden** Zweigen. `apps/api/src/routes/content.ts:793` gibt `accepted.data` bereits unverändert durch — kein Route-Code nötig, nur der Contract/die Typannahme im Frontend.
...
3. Accept-Aufruf-Stelle in `erstellen.vue` (wo `POST /v1/text-workshop/candidates/:id/accept` aufgerufen wird): nach erfolgreichem Accept, wenn `stylePresetId` gesetzt ist, für jede zurückgegebene `postMediaId` `POST /v1/post-media/:id/style-render` mit dieser `stylePresetId` aufrufen (sequenziell oder parallel, Fehresultat pro Foto einzeln anzeigen — ein fehlgeschlagenes Foto darf die anderen nicht blockieren). Zwischenzustand „Beitrag wird gestylt…" anzeigen, danach zur bestehenden Freigabe-Weiterleitung.
   ...
   - Dritter Testfall (Idempotenz-Pfad): `request-approval` künstlich fehlschlagen lassen, erneut auf „Übernehmen" klicken (löst laut `erstellen.vue:294-296` denselben Accept-Aufruf mit `alreadyAccepted=true` aus) — die zweite Antwort muss dieselben `postMediaIds` liefern wie die erste, und der Style-Render-Schritt muss auch bei diesem Retry ausgeführt werden.
