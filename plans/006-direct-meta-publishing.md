...
**Teilweise bereits umgesetzt in Paket 025** (`plans/025-inhalts-pipeline-entwurf-und-veroeffentlichung.md`): `POST /v1/publications/:id/execute` ruft `SocialPublisher.publish()` jetzt tatsächlich auf (Compare-and-Set, Token-Entschlüsselung, `publication_attempts`-Aufzeichnung, Audit) — vorher tat das im gesamten Repository kein Code. `GET /v1/media-grants/:token` liefert die in Abschnitt 3 verlangte sichere Medienübergabe. Kein Hatchet-Cron (Paket 004 weiterhin nicht produktiv): der Endpunkt ist ein expliziter, synchroner Trigger, kein automatisches Ausführen zu einem künftigen `scheduled_for`-Zeitpunkt. Ohne die weiterhin fehlende Upload-/Freigabepipeline (002/003) hat jede so entstehende `post_version` keine Medien — `FakePublisher`/`MetaPublisher` lehnen das unconditional ab, ein echter Veröffentlichungsversuch bleibt deshalb bis 002/003 ungetestet. Retry-/Reconciliation-Verhalten über einen einzelnen Versuch hinaus und die Phase-0-Sandbox-Nachweise bleiben für 006 offen.
