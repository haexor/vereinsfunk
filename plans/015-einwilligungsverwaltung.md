...
- ~~**Aufbewahrung von Einwilligungsnachweisen**...~~ **Entschieden (2026-08-08): Standardwert 5 Jahre ab Ende der Gültigkeit, pro Verein konfigurierbar von 1 bis 30 Jahren (Plan-Vorschlag übernommen).** `consent_evidence_years` wird über den PUT-Endpunkt pro Verein gespeichert und vom Retention-Lauf verwendet; das Setzen erfordert `organization.manage`. Paket 020 kann die Frist mit einem manuell auslösbaren Retention-Lauf anwenden; ohne wiederkehrende, überwachte Scheduler-Registrierung ist die Aufbewahrung noch nicht vollständig automatisch technisch durchgesetzt. Paket 004 liefert nur den technischen Outbox-/Worker-Unterbau.
