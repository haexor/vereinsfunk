...
- ~~**Aufbewahrung von Einwilligungsnachweisen**...~~ **Entschieden (2026-08-08): 5 Jahre ab Ende der Gültigkeit (Plan-Vorschlag übernommen).** Diese Sitzung setzt die Frist nicht technisch durch — kein Löschjob existiert (Paket 004/Cron weiterhin „in Arbeit“, siehe unten) — sie ist als Entscheidung festgehalten, damit Paket 020 sie direkt umsetzen kann, statt sie noch treffen zu müssen.
...
- **Kein Cron für „läuft in 30 Tagen ab“** und keine automatische Aufbewahrungslöschung — dieselbe Einschränkung wie bei `mark_stalled_approval_stages()` (011), `recompute_directory_minor_status()` (014) und den Anlassvorschlägen (019): Paket 004 (Hatchet) ist weiterhin „in Arbeit“. `evaluateConsent`/`computeConsentRecordStatus` berechnen `expiring_soon`/`expired` live bei jedem Aufruf statt über einen Job — funktional gleichwertig für die Anzeige, nur ohne proaktive Benachrichtigung.
...
- `supabase/tests/consent_management.test.sql`: 28 pgTAP-Assertions (CHECK-Constraints inkl. Selbstablösung und Origin/Source-Kopplung, RLS inkl. Fremdverein-Ausschluss für `consent_requests`/`organization_consent_texts`, `consent.manage` in `has_department_permission`/`has_organization_permission`, die volle Widerrufskaskade über einen per SQL nachgestellten Beitragspfad — inklusive Nutzprobe des in diesem Paket behobenen Bugs in `invalidate_approvals_for_media_change` —, Löschverhalten `restrict`/`set null`) — 436 pgTAP-Tests insgesamt, 14 Dateien.
