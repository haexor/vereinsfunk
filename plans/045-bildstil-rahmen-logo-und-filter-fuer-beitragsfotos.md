...
2. **Zwei echte Fehler per echtem Playwright-Lauf gegen die lokale App gefunden**, die die eigene pgTAP-Suite nicht gefangen hatte, weil die betroffenen Schreibzugriffe dort ausschließlich unter der Rolle `postgres` liefen, nie unter `authenticated`:
