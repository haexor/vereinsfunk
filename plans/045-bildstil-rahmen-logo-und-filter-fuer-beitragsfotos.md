...
1. **Asset-Re-Upload zerstörte bestehende Presets.** `POST /v1/brand/assets` löste beim Hochladen bislang jedes vorherige `'ready'`-Asset derselben Kind/Ebene ab (`status='replaced'`) — richtig für Icon/Wortmarke (ein Platz pro Ebene), falsch für `frame`/`watermark`: `image_style_presets` zeigt per fester ID auf ein bestimmtes Asset, und ein Verein soll mehrere Rahmen-/Wasserzeichen-Varianten parallel pflegen können. Jeder weitere Upload hätte jedes bestehende Preset, das noch auf das abgelöste Asset zeigt, beim nächsten Speichern mit `invalid_asset_reference` scheitern lassen, ohne Ausweg in der UI. Ein erster Fix beschränkte das Supersede auf `logo_mark`/`wordmark` — eine zweite Review-Runde auf genau diesem Fix-Commit fand, dass `department_brand_profiles.logo_asset_id`/`team_brand_profiles.logo_asset_id` `logo_mark`/`wordmark` genauso per fester ID referenzieren wie `image_style_presets` `frame`/`watermark`, derselbe Bug also für zwei von vier erreichbaren Kinds offenblieb. Endgültiger Fix: kein über die generische Route erreichbares Kind wird noch superseded (`apps/api/src/routes/brand.ts`).
...
3. **Leere Zahlenfelder umgingen die Validierung.** `v-model.number` auf `frameWidthPx`/`logoSizePercent`/`logoMarginPercent` lieferte bei leerem Feld einen leeren String statt `null` (Vue-`looseToNumber`-Fallback) — `isValid`s `=== null`-Prüfung erkannte das nicht, der Speichern-Knopf blieb aktiv, die Zod-Validierung scheiterte erst serverseitig mit einer generischen Fehlermeldung. Fix: gleiches manuelle `@input`-Handling wie bei `frameCornerRadiusPx` (leerer String → `null`), später in eine gemeinsame `nullableNumberFromInput`-Hilfsfunktion gezogen statt vierfach inline wiederholt.
4. **Veraltete Asset-Referenz beim Ebenenwechsel.** Ein Wechsel der aktiven Ebene im Anlage-Formular aktualisierte die wählbaren Assets, ließ aber eine bereits gewählte `frameBrandAssetId`/`logoBrandAssetId` der alten Ebene im Entwurf stehen — fiel erst beim Speichern als `invalid_asset_reference` auf. Ein erster Fix setzte dafür den gesamten Anlage-Entwurf und eine offene Bearbeitung zurück — eine zweite Review-Runde befand das als unnötig grobschlächtig (unabhängige Eingaben wie Name/Filter gingen bei einem bloßen Tab-Wechsel verloren, eine laufende Bearbeitung verlor ihre Fortsetzbarkeit). Endgültiger Fix: `selectScope()` nullt ausschließlich die beiden betroffenen Asset-Felder, wenn sie in der neuen Ebene nicht mehr wählbar sind; eine laufende Bearbeitung bleibt unberührt (sie blendet sich über `ownPresets` ohnehin aus, sobald ihre Ebene nicht mehr aktiv ist).
...
## Umsetzung: Ergebnis und Abweichungen (PR 2, 2026-08-19)

Sharp-Compositing-Engine, neue Route und atomare RPC vollständig umgesetzt und verifiziert: 12 Pixelproben-Tests (`apps/api/src/imageStyle.test.ts`, echtes `sharp` gegen generierte Fixture-Bilder, kein Mock) für jeden Filter, den parametrischen Rahmen (inkl. abgerundeter Ecken, Farb-Rollen `primary`/`accent`), die eigene Rahmengrafik, das Logo-Wasserzeichen an allen fünf Positionen und die Kombination aller drei Schritte; 6 neue Route-Tests (`apps/api/src/imageStyle.routes.test.ts`) für die Ablehnungspfade (nicht gefunden, keine `post.edit`-Berechtigung, bereits zur Freigabe eingereicht, Preset nicht gefunden/nicht wählbar) und den vollständigen Erfolgspfad inkl. RPC-Argumenten und Recipe-Inhalt; 9 pgTAP-Assertions für die neue Funktion `apply_image_style_render` (Editierbarkeits-Gate, atomare Derivat-Erzeugung, idempotenter Retry ohne die Immutability-Sperre zu verletzen, zwei Beiträge mit demselben Ursprungsfoto bleiben unabhängig) — verifiziert per rollback-gekapseltem Testlauf gegen die lokale Supabase-Instanz. `pnpm check` grün. Nach dem Review-Nachtrag unten: 18 Engine-Tests, 18 Route-Tests, 11 pgTAP-Assertions; `pnpm check` und `pnpm db:test` (946 Assertions über 36 Dateien) grün.

Abweichungen bzw. Ergänzungen gegenüber diesem Plandokument:

1. **Duoton hat keine direkte Sharp-Entsprechung.** `.tint()`/`.modulate()` etc. reichen für `vereinsfarben_duoton` nicht — Sharp kennt keinen Gradient-Map-Filter. Umgesetzt als manuelle Pixel-Interpolation: `.greyscale().raw()` liefert die Luminanz je Pixel, die dann von Hand zwischen `primaryColor` (Schatten) und `accentColor` (Lichter) interpoliert und über `sharp(rawBuffer, {raw: {...}})` neu kodiert wird.
2. **`warm` ist eine konkrete Farbentscheidung.** Der Plan nennt nur "Richtung Amber"; umgesetzt als Verschiebung je Kanal (`.linear([1.06,1.0,0.94],[10,2,-8])`), eine feste, dokumentierte Wahl statt eines Reglers. Bewusst *nicht* `.tint()`: das koloriert und hätte jedes Foto einfarbig sepia gemacht — praktisch ein zweites `schwarz_weiss`.
3. **`frameColor`/Duoton lösen gegen die tatsächlich vererbte Vereinsfarbe der Beitrags-Ebene auf, nicht gegen einen UI-Zustand.** `packages/domain`s `resolveBrand()` (bereits für die Marke-Seite vorhanden) wird hier erstmals serverseitig verwendet, um Vereins-/Abteilungs-/Mannschaftsprofil zur effektiven `primaryColor`/`accentColor` der Post-Ebene aufzulösen. `OrganizationBrandLevel`/`DepartmentBrandLevel` sind dafür jetzt aus `packages/domain` exportiert, statt sie im API-Paket nachzubilden.
4. **Die Route autorisiert über `post.edit`, nicht `brand.manage`.** Das Anwenden eines Bildstil-Presets auf ein konkretes Foto ist eine Beitrags-Aktion (wer den Beitrag bearbeiten darf, darf auch sein Foto stylen), keine Marken-Verwaltungsaktion — anders als PR 1s CRUD-Routen für die Presets selbst.
5. **`apply_image_style_render` leitet die `post.edit`-Berechtigung nicht selbst aus einer Akteur-ID her**, anders als `accept_text_generation_candidate`s eigener (loser, nicht rollenspezifischer) Mitgliedschafts-Check. Die volle, mehrfach kaskadierende Rollen-Berechtigungslogik aus `authz.has_department_permission` basiert auf `auth.uid()` und ist unter Service Role nicht auswertbar — sie in SQL zu duplizieren wäre ein eigenes Wartungsrisiko gewesen. Die Route prüft `post.edit` stattdessen vorher über den Nutzer-eigenen Client (echter `auth.uid()`-Kontext); die Funktion selbst ist ausschließlich Service-Role-erreichbar (nicht `authenticated`) und wiederholt nur die race-anfällige, zeitlich variable Prüfung (Post-Status), nicht die statische Rollenprüfung.
6. **Kein Aufräumen verwaister Assets.** Ein zunächst hochgeladenes, dann von keinem Preset mehr referenziertes `frame`/`watermark`-Asset (PR 1) oder ein durch Preset-Löschung verwaistes gestyltes Derivat wird nicht automatisch entfernt — bewusst zurückgestellt, gleiche Haltung wie beim Nicht-Aufräumen alter Marken-Assets.
7. **Nicht in `erstellen.vue` verdrahtet.** Diese Route ist end-to-end funktionsfähig, aber ohne Aufrufer in der Oberfläche — das ist PR 3s Aufgabe.

### Nachtrag aus dem Code-Review dieses PRs

Fünfzehn bestätigte Befunde behoben; die inhaltlich tragenden:

- **`post_media`/`post_versions`/`media_derivatives` werden über die Service Role gelesen.** Ihre `SELECT`-Policies gewähren nur `authz.is_organization_member`, und `accept_invitation` legt für eine Abteilungs-/Mannschaftseinladung gar keine `organization_memberships`-Zeile an — genau die Redakteure, für die diese Route da ist, hätten ein 404 bekommen, bevor die Berechtigungsprüfung überhaupt lief. Sichtbarkeit hängt jetzt an `posts_select` (Nutzer-Client) plus `post.edit`.
- **Nur die aktuelle Fassung ist stylebar.** Ältere `post_versions` behalten ihre `post_media`-Zeilen, während der Beitrag nach einer neuen Fassung wieder `draft_ready` ist — ohne die Prüfung `post_media.post_version_id = posts.current_version_id` ließ sich der bereits freigegebene und veröffentlichte Bildstand nachträglich umschreiben (409 `post_version_not_current`).
- **Ausgabeformat wird bewusst gewählt statt geerbt.** Zwischenschritte müssen nach PNG kodieren; als Endformat blähte das ein JPEG-Foto gemessen um das ~12-fache auf und hätte gerade die gestylten Fotos an den Größenlimits der Veröffentlichungsziele scheitern lassen. Jetzt: PNG nur bei Transparenz oder PNG-Quelle, sonst JPEG. Das hält das Ergebnis zugleich immer innerhalb der `allowed_mime_types` des `rendered-media`-Buckets — eine WebP-Quelle hätte vorher einen `image/webp`-Upload und damit einen Storage-Fehler erzeugt.
- **Logo-Platzierung kann das Bild nicht mehr verlassen.** Größe und Rand beide an der Bildbreite zu messen ergab bei einem Panorama einen negativen Offset; `sharp`s `composite` wirft darauf, und die Route hat kein `try/catch` — ein Preset innerhalb der DB-CHECK-Grenzen (30 %/15 %) wurde so zum 500. Der Rand bezieht sich jetzt auf die kürzere Kante, die Logo-Box zusätzlich auf den verbleibenden Platz.
- **`apply_image_style_render` macht Compare-and-Set auf den Derivat-Zeiger** (`p_expected_media_derivative_id`, sonst `post_media_changed`): das `for update` reihte zwei parallele Läufe zwar, ließ aber die Commit-Reihenfolge entscheiden, welches Preset am Ende hing — beide Aufrufer bekamen 201 mit ihrem eigenen Ergebnis. Die Fehler der Funktion bildet die Route jetzt auf 404/409 ab statt sie als 500 durchzureichen.
- **Kein Video in `sharp`.** `media_assets` nimmt `video/mp4` an und keine der vorgelagerten Prüfungen schaut auf den Typ — jetzt 422 `source_media_asset_not_an_image` statt 500.
- **Eckenradius gilt für jeden Rahmentyp**, so wie Contract, DB-CHECKs und die CSS-Live-Vorschau ihn schon immer erlaubten; vorher rundete nur der parametrische Zweig, die Vorschau versprach also etwas, das im Ergebnis fehlte.
- **`editablePostStatuses`/`isPostEditable` liegen in `packages/domain`** statt als dritte Inline-Kopie in der Route, und die DB-Lesestellen der neuen Route gehen über Zod statt über `as string` (`media_assets.sha256` ist nullable und wäre als `null` in den Rezept-Snapshot gelaufen).

**Migrations-Zeitstempel:** `main` hat durch PR #113 zwischenzeitlich `2026081915`/`2026081916` belegt (Umnummerierung der Migrationen aus PR #111). Die Migration dieses PRs heißt deshalb jetzt `2026081918_image_style_render.sql`, und `main` ist in den Branch gemergt — der Guard prüft die Reihenfolge gegen `origin/main` und sieht die Kollision erst nach diesem Merge.
