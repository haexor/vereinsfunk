...
2. **`warm` ist eine konkrete Farbentscheidung.** Der Plan nennt nur "Richtung Amber"; umgesetzt als `.tint({r:255,g:200,b:150})`, eine feste, dokumentierte Wahl statt eines Reglers.
3. **`frameColor`/Duoton lösen gegen die tatsächlich vererbte Vereinsfarbe der Beitrags-Ebene auf, nicht gegen einen UI-Zustand.** `packages/domain`s `resolveBrand()` (bereits für die Marke-Seite vorhanden) wird hier erstmals serverseitig verwendet, um Vereins-/Abteilungs-/Mannschaftsprofil zur effektiven `primaryColor`/`accentColor` der Post-Ebene aufzulösen — nicht in der Domain-Paket-Oberfläche exportiert (`OrganizationBrandLevel`/`DepartmentBrandLevel`), deshalb lokal über die exportierten Basisprofile (`BrandLevelProfile`/`BrandOverrideProfile`) nachgebildet.
