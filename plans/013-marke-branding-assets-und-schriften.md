...
- **Beim Code-Review der Pull-Request gefundene und behobene Punkte** (eigenes Review plus CodeRabbit, danach je Punkt entschieden statt pauschal übernommen):
  - **Der Rahmen einer höheren Ebene galt nur beim Auflösen, nicht beim Schreiben.** `PUT /v1/departments/:id/brand` und `PUT /v1/teams/:id/brand` speicherten Farben, Tonalität und Asset-Referenzen auch dann, wenn der Verein `allow_department_overrides = false` gesetzt oder das Feld gesperrt hatte. `resolveBrand` ignorierte den Wert zwar zuverlässig — die Abteilung sah ihn aber gespeichert im Formular stehen und nirgends wirken. Beide Endpunkte lehnen das jetzt mit `overrides_not_allowed` bzw. `field_locked` ab; `null` (auf geerbt zurücksetzen) bleibt immer erlaubt.
  - **`resolveBrand` ließ eine Mannschaft abweichen, obwohl der Verein Abteilungsabweichungen verboten hatte** — die Abteilung konnte über ihr eigenes `allow_team_overrides` die Vereinssperre wieder öffnen. `allowDepartmentOverrides` ist jetzt auch für die Mannschaftsebene Voraussetzung.
  - **`locked_fields` nahm beliebige Zeichenketten an.** Ein Tippfehler (`primary_colour`) wurde gespeichert und sperrte lautlos nichts; umgekehrt bot die Oberfläche Sperren für `displayFontKey`/`bodyFontKey` an, die unterhalb des Vereins ohnehin keine Spalte haben. Der sperrbare Satz ist jetzt an genau einer Stelle definiert (`BRAND_LOCKABLE_FIELDS`) und in Vertrag, Domäne, Oberfläche und einem SQL-CHECK durchgesetzt.
  - **Vereinsweite Schrift-Referenzen waren nicht auf vereinsweite Assets begrenzt.** `brand_profiles_update` (aus Paket 009) kannte die neuen Spalten nicht; per direktem PostgREST-Zugriff ließ sich das Schrift-Asset *einer* Abteilung als vereinsweite Schrift eintragen und damit allen Schwesterabteilungen zugänglich machen — genau die Grenze, die `authz.brand_asset_is_selectable()` unterhalb schon zog. Die Policy prüft beide Spalten jetzt mit, zwei neue pgTAP-Tests decken es ab.
  - **Dieselbe Datei ein zweites Mal hochzuladen zerstörte den Asset-Zustand.** `object_path` trägt den Inhalts-Hash und ist per `unique (bucket_id, object_path)` eindeutig; der Supersede-Schritt lief aber vorher. Ergebnis: die vorhandene Zeile stand auf `replaced`, das anschließende `insert` scheiterte, und der Verein hatte danach gar kein aktives Logo-Asset mehr — sichtbar als rohe 500. Beide Upload-Endpunkte nutzen jetzt `upsert` auf `(bucket_id, object_path)`.
  - **Die Lizenzbestätigung entwertete alle anderen eigenen Schriften derselben Ebene.** Der Supersede lief über `kind = 'font'` allein, obwohl das Markenprofil eine Überschriften- *und* eine Fließtextschrift kennt (und der Plan mehrere Schriftdateien je Verein vorsieht). Abgelöst wird jetzt nur dieselbe Schrift (Familie, Schnitt, Lage).
  - **Eine eigene Schrift ließ sich nicht als Fließtextschrift wählen** — die Oberfläche bot nur „als Überschriften-Schrift“ an, obwohl `bodyFontAssetId` durchgereicht und validiert wurde. Beide Rollen sind jetzt wählbar und per erneutem Klick wieder abwählbar.
  - **`overrideFor()` schrieb reaktiven Zustand aus einem `computed` heraus.** Lesen und Anlegen sind getrennt (`readOverride` vs. `overrideFor`); angelegt wird nur noch in Event-Handlern. Das ist zugleich ein Teil der unten beschriebenen Hydration-Abweichung.
  - Kleinere, ebenfalls behobene Punkte: `width`/`height` eines Logos wurden vor `.rotate()` gelesen und waren bei EXIF-Orientierung 5–8 vertauscht; die SVG-Rasterisierung hatte keine Pixelgrenze (ein SVG mit sehr großer `viewBox` konnte den Prozess an sein Speicherlimit bringen) und meldet Fehler jetzt als 400 statt 500; `relativeLuminance` lieferte bei einer Kurzform wie `#abc` still `NaN`; der Logo-Watcher in `layouts/default.vue` und der Schrift-`watchEffect` auf `/marke` konnten veraltete Ergebnisse schreiben; `ClubPost` erzeugte den `delayRender`-Handle erst nach dem ersten Commit und ließ eine fehlgeschlagene Schrift als unbehandelte Promise-Rejection stehen; der Scope-Umschalter zeigte den aktiven Bereich nur über Farbe.
  - **Bewusst nicht übernommen**: (a) `OrganizationBrandUpdateSchema` verlangt die neuen Farbrollen als Pflichtfelder — das ist eine gewollte Vertragsänderung, der einzige Client liegt im selben Repository und wird in derselben Pull-Request mitgezogen, und die Spalten sind `not null` mit Default, ein Teil-Payload müsste also Werte erfinden. (b) `ClubPost` nimmt Schrift- und Logo-URLs weiterhin als `z.url()` statt als autorisierte Asset-Referenz entgegen: es gibt heute keinen Aufrufer, der Nutzereingaben dorthin reicht (`Root.tsx` setzt ausschließlich statische Defaults), und der Renderpfad, der signierte URLs erzeugt, entsteht erst mit der Inhalts-Pipeline — dort gehört die Grenze hin, nicht in eine spekulative Vorwegnahme.
- **Beim manuellen Browser-Test gefunden, vorbestehend und außerhalb des Umfangs dieses Pakets**: `/marke` zeigt im Entwicklungsserver einen Vue-Hydration-Mismatch in der Browser-Konsole (Farben/Kontrastwerte/Logo weichen zwischen dem serverseitig gerenderten und dem client-hydrierten Zustand kurz voneinander ab, bevor Vue automatisch korrigiert). Ursache ist **nicht** dieses Paket: `useSession()`/`useScope()` sind laut eigenem Code-Kommentar (`apps/web/app/composables/useScope.ts`) serverseitig grundsätzlich leer, jede organisationsbezogene Seite rendert auf dem Server deshalb einen Leer-/Defaultzustand. Derselbe Mismatch tritt nachweislich auch in `layouts/default.vue` auf (die Sidebar-Initialen „LM“/„Lena Müller“), betrifft also die ganze authentifizierte App, nicht nur die neue Markenseite — sie fällt hier nur stärker auf, weil sie mehr bedingt gerenderte Blöcke hat. Funktional harmlos (Vue korrigiert die Hydration automatisch, alle Endergebnisse im manuellen Test waren korrekt), aber ein sichtbares Rauschen in der Konsole. Ein Versuch, das lokal per `definePageMeta({ ssr: false })` zu beheben, zeigte im Playwright-Test keine Wirkung (unklar, ob am Entwicklungsserver-Modus liegend oder an einer fehlenden Zusatzkonfiguration) und wurde deshalb wieder verworfen, statt eine unbestätigte Lösung stehen zu lassen. Eine echte Behebung braucht eine serverseitige Sitzungsauflösung (z. B. über einen Auth-Cookie) und ist ein eigenes, app-weites Vorhaben.
