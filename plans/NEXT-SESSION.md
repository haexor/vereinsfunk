...
- Tests: `apps/web/package.json` verwendet jetzt `vitest run` ohne `--passWithNoTests`. Sechs Testdateien führen vierzehn Tests aus. Neben `security`, `apiClient`, `apiClientMigration` und `channelComposition` sichern `useBrandOverrides.test.ts` die explizite Anlage von Marken-Overrides und `memberDates.test.ts` die Vereinszeitzone bei Mitgliedschafts-Befristungen ab.
...
5. Weitere gezielte Web-Tests für einen erfolgreichen Load und einen Berechtigungsfehler ergänzen. Die Mutation ist jetzt im API-Client testbar; das Mengen-Kriterium („mindestens fünf Testdateien“) ist mit sechs Dateien erfüllt.
