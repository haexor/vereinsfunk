...
## Offener sicherheitsrelevanter Punkt aus 011

`request_approval` prüft nicht, ob die vom Aufrufer genannten Prüfer die in der Richtlinie **konfigurierten** sind — nur Struktur, Vereinsmitgliedschaft und das Vorhandensein der Minderjährigenstufe. Wer `post.submit` hat, kann die RPC direkt aufrufen und seine Freigabe an einen selbst gewählten Vereinskollegen richten, auch bei `review_mode = 'named'`, und die Minderjährigenstufe mit einer beliebigen Person besetzen. Die Entscheidung darüber (Routenauflösung in SQL nachbauen, oder den Grant auf `service_role` zurücknehmen und in Fastify durchsetzen) ist in `plans/024-freigaberoute-neu-aufloesen.md`, Abschnitt 2 aufgeschrieben und gilt für `request_approval` und die geplante Neuauflösungs-RPC gemeinsam. **Vor 024 zu klären, und vor dem ersten echten Verein mit Minderjährigen-Inhalten.**
