...
- Tests: `apps/web/package.json` verwendet jetzt `vitest run` ohne `--passWithNoTests`. Fünf Testdateien führen zwölf Tests aus. Neben `security`, `apiClient` und `apiClientMigration` sichern `useBrandOverrides.test.ts` die explizite Anlage von Marken-Overrides und `memberDates.test.ts` die Vereinszeitzone bei Mitgliedschafts-Befristungen ab.
...
- `marke.vue` (497 LoC) und `mitglieder.vue` (482 LoC) liegen jetzt unter dem 500-LoC-Kriterium. Marke kapselt Asset-/Vorschau-/Upload-State in `useBrandAssets` und Override-State in `useBrandOverrides`; Mitglieder kapselt die Rollen-, Befristungs- und Vertrauensdarstellung in `MemberList`. `memberDates.ts` korrigiert dabei einen Millisekunden-Randfall: Ablaufzeitpunkte liegen am lokalen Tagesende, nicht knapp danach. `kanaele.vue` (561 LoC) und `einstellungen/recht.vue` (594 LoC) stehen noch aus. Bei Marke werden temporäre Logo-Object-URLs beim Ersetzen und beim Unmount widerrufen.
...
3. Paket 028 fortsetzen: `kanaele.vue` und `einstellungen/recht.vue` in fachliche Composables/Komponenten aufteilen. Danach entscheiden, ob `useIntegrationSources` noch verhältnismäßig ist; keine abstrakte Hülle nur für einen Aufrufer bauen.
4. Die verbleibenden zwei großen Seiten unter 500 LoC bringen, Zielwert 250 LoC. Keine API-Verträge, Auth, Cookie-Namen oder Routing ändern; keine Service Role im Browser.
5. Weitere gezielte Web-Tests für einen erfolgreichen Load, einen Berechtigungsfehler und eine Mutation ergänzen. Das Mengen-Kriterium („mindestens fünf Testdateien“) ist mit fünf Dateien erfüllt.
