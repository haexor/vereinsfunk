...
- Paket 028 ist erledigt. `apps/web/app/composables/useApiClient.ts` delegiert an `apps/web/app/utils/apiClient.ts`: API-Basis-URL, aktueller Bearer-Header ohne Session-Cache, optionale Zod-Validierung und typisierte Fehlercodes sind zentral. Öffentliche Anfragen setzen `authenticate: false`.
...
- `integrationen.vue` ist nach der Zerlegung bei 440 LoC. State und API-Logik liegen dort noch in der Seite; ein `useIntegrationSources`-Composable wurde bewusst nicht eingeführt, weil die Seite sein einziger Aufrufer wäre. Die Formularkomponenten erhalten State als Props und senden Events, ohne selbst API-Aufrufe auszuführen.
...
- Authentifizierter lokaler Browser-Smoke-Test für Marke, Mitglieder, Integrationen, Kanäle und Recht ist erledigt: frischer lokaler Supabase-Reset, ausschließlich Wegwerf-Env-Dateien außerhalb des Worktrees, keine unerwarteten Konsolen-/Netzwerkfehler. Marke, Mitglieder und Integrationen führten lokale Testmutationen aus; Recht führte nur den zerstörungsfreien Testlauf aus. Der erwartete Fehler beim nicht konfigurierten Meta-Connect und der Berechtigungshinweis für ein Konto ohne Vereinsmitgliedschaft waren verständlich.
...
3. Paket 028 nicht weiter abstrahieren: Die Done-Kriterien sind erfüllt und `useIntegrationSources` hätte nur einen Aufrufer.
4. Für Paket 027 die Route-Module in `apps/api/src/app.ts` fachlich zerlegen; keine API-Verträge, Auth, Cookie-Namen oder Routing ändern und keine Service Role im Browser verwenden.
5. Paket 029 kann nach Paket 027 parallel vorbereitet werden; fachliche Contracts-/Domain-Exports nicht mit Paket 028 vermischen.
6. Bei künftigen UI-Änderungen einen manuellen Browser-Smoke-Test mit lokaler Wegwerf-`.env` gemäß `plans/README.md` durchführen. Keine echte `.env` kopieren oder auslesen und temporäre Dateien danach entfernen.
...
Paket 028 ist abgeschlossen. Als Nächstes Paket 027 (Fastify-Routenmodule), danach Paket 029 (fachliche Contracts-/Domain-Exports); keine API-Verträge ohne begründete, dokumentierte Entscheidung ändern.
