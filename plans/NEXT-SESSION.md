...
- Fünf reine Domain-Funktionen in `packages/domain`: `resolveReviewRoute`, `evaluateSubmitPermission`, `evaluateQuota`, `resolveReviewers`, `resolveEffectiveConfig` (Wrapper um das bereits vorhandene `mergeEffectiveConfig`, das jetzt auch aus Produktionscode aufgerufen wird, nicht mehr nur aus Tests).
