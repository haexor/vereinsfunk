...
- **`GET /v1/organizations/:id/invitations` verifiziert die Scope-Kette aus `departmentId`/`teamId` nicht serverseitig** (dieselbe Klasse wie der behobene `POST`-Fund). Heute kein Leck, weil der zusammengesetzte Fremdschlüssel auf `invitations` eine fremde Kombination ohnehin auf null Zeilen filtert — die Sicherheit hängt damit aber am Fremdschlüssel, nicht an einer Prüfung.
