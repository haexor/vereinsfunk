...
- Die aufklappbare Detailebene je Mitglied ist wie geplant nicht Teil dieses Pakets — sie entsteht in Paket 023 und wird von 011 gefüllt.
...
- **Mitglieder-Detailebene** (Rolle, Befristung, Einreichrecht, Prüfpflicht, Freigaberoute je Person) ist wie ursprünglich geplant nicht Teil dieses Pakets: sie entsteht in Paket 023, Paket 011 füllt sie.
...
- **Wer einladen darf, ist heute nur über die Rollenvergabe steuerbar**, nicht über eine eigene Richtlinie: `member.invite` steckt fest in `department_admin` und `team_manager`. Der Verein bestimmt damit indirekt (durch die Rollenvergabe), welche Personen einladen dürfen, kann es aber einer Abteilung nicht rollenunabhängig entziehen. Das ist ein Vererbungsfall für `policy_settings`. **Entscheidung des Nutzers: in Paket 023 bauen** — als `invite_allowed boolean` (`null` = erben), dort im Datenmodell vermerkt.
