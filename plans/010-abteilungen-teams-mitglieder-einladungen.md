...
- **`POST /v1/invitations/:id/revoke` widerruft auch eine bereits angenommene Einladung** (setzt `revoked_at` auf einer angenommenen Zeile). Ohne Wirkung auf die Mitgliedschaft und in der Oberfläche nicht sichtbar, aber ein irreführender Audit-Eintrag.
- **`invitations` ist für `authenticated` spaltenblind schreibbar**: `grant insert, update on table public.invitations to authenticated` plus die Policy `invitations_update` erlauben jedem Inhaber von `member.invite`, `token_hash`, `email` oder `expires_at` einer fremden offenen Einladung direkt über PostgREST zu ändern — an der API und damit am Audit-Trail vorbei. Keine Rechteausweitung (`invitations_update` prüft weiter `can_assign_role`, die Rolle bleibt also unter der eigenen Decke), aber unnötige Angriffsfläche. Der Insert-Grant ist inzwischen sogar toter Code: `POST /v1/invitations` geht über `create_invitation()` (`security definer`). Saubere Verschärfung: Insert-Grant und `invitations_insert`-Policy entfernen, Update auf `grant update (revoked_at)` einschränken. Nicht in diesem Durchgang gemacht, weil die pgTAP-Suite die Insert-Policy direkt testet (`structure_and_invitations.test.sql:132-223`) und die Tests dafür umgebaut werden müssten.
