...
- **Wer einladen darf, ist heute nur über die Rollenvergabe steuerbar**, nicht über eine eigene Richtlinie: `member.invite` steckt fest in `department_admin` und `team_manager`. Der Verein bestimmt damit indirekt (durch die Rollenvergabe), welche Personen einladen dürfen, kann es aber einer Abteilung nicht rollenunabhängig entziehen. Das ist derselbe Vererbungsfall wie `submit_requires_permission` in `policy_settings` (Paket 011, „Vererbung: eine Richtung“) und gehört dort hin, nicht in dieses Paket. **Entscheidung des Nutzers: in Paket 011 bauen** — als `invite_allowed boolean` (`null` = erben), dort im Datenmodell vermerkt.
