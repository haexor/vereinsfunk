...
Der Workflow-Name `cleanup-expired-invitations` ist in `WorkflowNameSchema` registriert und wird vom Worker als technische Task-Definition registriert, **der eigentliche fachliche Hatchet-Cron ist aber nicht gebaut** — dieselbe Situation wie `collect-analytics` (Paket 017) im bestehenden Rückbau-Inventar: ein registrierter Workflow ohne fachlichen Cron- oder Outbox-Trigger ist ein etabliertes, akzeptiertes Muster in diesem Repo, kein Rückbau-Verstoß. Paket 004 ist als technischer ID-only Outbox-/Worker-Unterbau abgeschlossen; ein periodischer Cron-Trigger für dieses Aufräumen bleibt eine separate fachliche Verdrahtung. **Nachtrag Code-Review**: die ursprünglich hier dokumentierte Konsequenz — eine abgelaufene, nie angenommene Einladung blockiert über `invitations_open_unique` eine neue Einladung derselben Adresse, bis sie manuell widerrufen wird — ist inzwischen unabhängig vom Cron behoben. `POST /v1/invitations` ruft dafür nicht mehr direkt `insert()` auf, sondern die security-definer-RPC `public.create_invitation(...)`: sie sperrt eine vorhandene offene Zeile für denselben Scope/dieselbe Adresse per `for update`, verwirft sie in derselben Transaktion, falls sie bereits abgelaufen ist, und legt erst danach die neue Einladung an. Der fehlende Cron bleibt trotzdem sinnvoll (Aufräumen der Tabelle selbst), ist aber kein Funktionsblocker mehr.
...
- Kein Rollenwechsel-Button (nur Entfernen) — `PATCH /v1/memberships/:id` ist vollständig implementiert und getestet, die UI ruft ihn nur noch nicht auf. **✓ Nachgezogen in Paket 023**: aufklappbare Detailebene je Mitgliedschaft mit Rollen-Select.
- Keine Befristung (`expires_at` setzen) aus der Oberfläche — die Spalte existiert und wird von allen `authz`-Funktionen bereits geprüft (siehe Datenmodell), nur kein UI-Formular dafür. **✓ Nachgezogen in Paket 023**: eigene RPC `set_membership_expiry()` plus `PATCH /v1/memberships/:id/expiry`, da es bis dahin auch serverseitig noch keinen Schreibpfad dafür gab.
- Die aufklappbare Detailebene je Mitglied ist wie geplant nicht Teil dieses Pakets — sie entsteht in Paket 023 und wird von 011 gefüllt. **✓ Erledigt**: Rolle und Befristung in Paket 023, Freigabe-Zuständigkeit und Vertrauen folgen in Paket 011.
...
- pgTAP: 50 Assertions in `supabase/tests/structure_and_invitations.test.sql` (38 aus der ursprünglichen Umsetzung, 12 aus dem Code-Review-Nachtrag für die drei neuen RPCs), zusammen mit den bestehenden Suiten 131 Assertions über 5 Dateien. Negativ abgesichert u. a.: Abteilungsadmin aus „Fußball“ kann keine Mitgliedschaft in „Handball“ anlegen; `team_manager` kann keine Abteilungsmitgliedschaft anlegen; `organization_admin` kann einen `organization_owner` weder entfernen noch (über `can_remove_role` direkt geprüft oder über `change_membership_role` versucht) degradieren; Löschen der letzten Abteilung/des letzten Inhabers schlägt fehl (Owner-Test jetzt in einem dedizierten Verein mit genau einem Inhaber, siehe Code-Review-Nachtrag); Löschen einer Abteilung/eines Teams mit Inhalt schlägt fehl; Einladung mit `organization_owner` oder scope-inkonsistenter Rolle verstößt gegen die Constraints; zweite offene Einladung für gleiche Adresse/Scope verstößt gegen den Unique-Index (aber `create_invitation` ersetzt eine bereits abgelaufene); `email_has_membership` verweigert einem nicht-berechtigten Aufrufer den Zugriff; das adressbezogene Rate-Limit greift auch nach einem `revoke()`; kompletter Einladungs-Annahme-Fluss inkl. Team-Einladung (automatische Abteilungs-`viewer`-Zeile), abgelaufener Einladung, E-Mail-Mismatch.
...
- API-Tests (`apps/api/src/app.test.ts`, 33 Tests): 403 für fehlende Berechtigung auf Struktur-/Mitgliedschafts-/Einladungsrouten (inkl. 403 statt eines stillen 204 bei einem durch RLS gefilterten `DELETE`, Code-Review-Nachtrag), 400 für scope-inkonsistente Rollen vor jedem DB-Zugriff, 409 für Kontingent-/Inhalts-/verantwortliche-Person-/letzter-Owner-Konflikte (letzteres jetzt über die atomare `change_membership_role`-RPC), 429 für Resend-Limits, 403/410 für Annahme-Fehler, und ein expliziter Test, dass das Rohtoken nie in einer API-Antwort auftaucht (nur in der — im Test abgefangenen — ausgehenden E-Mail).
...
- **Rollen als Teil des Unique-Keys** macht Rollenwechsel zu Löschen-und-Anlegen — im Audit-Log als ein kombinierter `audit_events`-Eintrag sichtbar (`membership.role_changed` mit `fromRole`/`toRole` in den Metadaten). **Nachtrag Code-Review**: ursprünglich liefen Löschen und Anlegen als zwei getrennte PostgREST-Aufrufe ohne gemeinsame Transaktion — ein fehlschlagender Insert verlor die bereits gelöschte alte Mitgliedschaft ohne Rollback, und ein durch RLS still gefiltertes Delete (0 getroffene Zeilen, kein Fehler) ließ zwei Mitgliedschaften nebeneinander bestehen. Behoben durch die security-definer-RPC `public.change_membership_role(...)`, die beide Schritte sowie dieselben `has_*_permission`-/`can_remove_role`-/`can_assign_role`-Prüfungen wie die `*_memberships_insert`/`_delete`-RLS-Policies in einer Transaktion ausführt.
...
- **Rate-Limit für erneutes Senden umgehbar** (behoben, siehe „Nachtrag Code-Review“ unten): `POST /v1/invitations/:id/revoke` gefolgt von einem neuen `POST /v1/invitations` für dieselbe Adresse setzte `send_count`/`last_sent_at` zurück, da beide Limits (429 bei `send_count ≥ 10`, DB-Trigger 1×/Stunde) nur an der jeweiligen **Einladungszeile** hingen, nicht an der Adresse über die Zeit. Ein Akteur mit `member.invite` konnte dadurch eine Zieladresse mit E-Mails fluten — erforderte bereits eine berechtigte Innentäterschaft (kein externer Angriff, kein Vorteil beim Erraten des Rohtokens, da jedes Token unabhängig 256 Bit Zufall ist), war beim Geheimnisse-Review dieses Pakets gefunden und zunächst als geringes Risiko zurückgestellt worden.

### Nachtrag Code-Review

Ein externes Code-Review (CodeRabbit) auf diesem Pull Request fand zusätzliche, teils kritische Funde, die alle vor dem Merge behoben wurden:

- **Adressbezogenes Rate-Limit umgesetzt**: neue Tabelle `public.invitation_send_counters` (Schlüssel: Verein, normalisierte E-Mail, Abteilung/Team) plus `authz.register_invitation_send(...)`, aufgerufen sowohl aus `create_invitation(...)` als auch aus `resend_invitation(...)`. Ein `revoke()` gefolgt von einem neuen `create()` umgeht das Limit damit nicht mehr — der Zähler überlebt den Lebenszyklus einzelner Einladungszeilen.
- **Rollenwechsel und Einladung-Anlegen sind jetzt atomare RPCs** (`change_membership_role`, `create_invitation`, `resend_invitation`) statt mehrerer getrennter PostgREST-Aufrufe — siehe die beiden Punkte oben.
- **Durch RLS gefilterte `DELETE`s lieferten 204 statt eines Fehlers**: PostgREST meldet keinen Fehler, wenn eine Policy die Zielzeile ausfiltert (0 getroffene Zeilen). Alle drei `DELETE`-Routen (Abteilung, Team, Mitgliedschaft) hängen jetzt `.select('id')` an und antworten mit 403, wenn keine Zeile zurückkommt.
- **Verantwortliche-Person-Guard** blockierte fälschlich auch das Entfernen einer reinen Team-/Abteilungsmitgliedschaft; gilt jetzt nur noch für `scope: 'organization'`.
- **`WEB_BASE_URL`** ist in Produktion jetzt Pflicht (verhinderte sonst einen stillen Fallback auf `localhost` in Einladungslinks); **SMTP-Zugangsdaten** werden jetzt an der Config-Grenze validiert, sobald `EMAIL_PROVIDER=smtp` gesetzt ist, unabhängig von `NODE_ENV`.
- **E-Mail-Versand** läuft jetzt in `try`/`catch` (mit Timeout auf dem SMTP-Transport) statt ungefangen auf dem Request-Thread; Zustellstatus (`emailDelivered`) landet in Antwort und `audit_events`.
- Diverse kleinere Funde: `MemberRoleEntrySchema` prüft jetzt Rolle gegen Scope-Ebene (analog zu den Mitgliedschafts-/Einladungs-Schemas), Barrierefreiheits-Ergänzungen (`aria-label`, Vorbelegung) auf `/mitglieder` und `/struktur`, Reload bei Vereinswechsel, getrennte Fehlermeldungen für Lade- vs. Aktionsfehler, korrigierte Löschgrund-Unterscheidung („letzte Abteilung“ vs. „hat Inhalt“), Paginierung der Mitgliederliste, `useCan`-Scope-Bug bei Team-Einträgen.
- **Mitglieder-Detailebene** (Rolle, Befristung, Einreichrecht, Prüfpflicht, Freigaberoute je Person) ist wie ursprünglich geplant nicht Teil dieses Pakets: sie entsteht in Paket 023, Paket 011 füllt sie. **✓ Rolle und Befristung erledigt in Paket 023** (2026-08-06); Einreichrecht/Prüfpflicht/Freigaberoute bleiben für Paket 011.

### Nachtrag zweites Code-Review

Ein eigenes Review auf demselben Pull Request fand die folgenden Punkte, alle behoben:

- **Team-Einladungen waren aus der Oberfläche unmöglich** (funktionaler Bruch): `pages/mitglieder.vue` sendete für eine Team-Einladung nur `teamId`, nie die Eltern-Abteilung. `CreateInvitationRequestSchema` verlangt beides, `resolveInvitationScope()` prüft beide gegeneinander — jede Team-Einladung endete deshalb mit 400 und der generischen Meldung „Die Einladung konnte nicht versendet werden.“ Der bereits vorhandene Helfer `departmentIdFor()` liefert die Eltern-Abteilung und speist jetzt auch die Payload. Abgesichert durch je einen negativen und positiven API-Test sowie das positive Gegenstück im Contracts-Test.
- **Abteilungs-/Teamverantwortliche konnten in der Oberfläche gar nicht einladen**: das Einladungsformular hing an `useCan('member.invite', { organizationId })`, also an vereinsweitem `member.invite`. Ein `department_admin` oder `team_manager` hat das nur in seinem eigenen Bereich (`has_department_permission`/`has_team_permission`) — für beide war das Formular komplett unsichtbar, obwohl API und RLS ihre Einladungen erlauben und `GET /v1/organizations/:id/invitations` extra um Scope-Parameter erweitert wurde, damit sie ihre offenen Einladungen sehen. Die Ebenen-, Abteilungs- und Team-Auswahl wird jetzt je Ebene über `useCan` gefiltert; das Formular erscheint, sobald mindestens eine Ebene übrig bleibt.
- **`/struktur` folgte dem Vereinswechsel nicht**: der Reload bei Wechsel des aktiven Vereins wurde nur auf `/mitglieder` ergänzt, obwohl beide Seiten `load()` genau einmal im Setup aufriefen.
- **Neue Abteilungen/Teams erschienen erst nach einem vollen Reload**: `authz.membership_scopes()` speist über `useSession()` den Abteilungswähler der Sidebar und die Ebenenauswahl auf `/mitglieder`. Ohne `refreshSession()` nach einer Struktur-Änderung war eine Einladung in ein frisch angelegtes Team nicht möglich, obwohl das Team auf `/struktur` sichtbar war.
- **Profilnamen der Mitgliederliste konnten still auf „Unbekannt“ fallen**: die drei Mitgliedschaftstabellen werden über `fetchAllRows()` geblättert, der Profil-Nachschlag lief aber als ein einzelnes `.in()` über alle Nutzer-IDs — doppelt begrenzt durch `max_rows=1000` und durch die URL-Länge (die IDs stehen als Query-String in der Anfrage und reißen schon bei einigen hundert UUIDs die Header-Grenze). Jetzt in Blöcken von 100, abgesichert durch einen API-Test mit 250 Mitgliedern.
- **`emailDelivered` wurde nie ausgewertet**: die API liefert den Zustellstatus, die Oberfläche meldete trotzdem pauschal „Einladung versendet.“ — bei fehlgeschlagenem SMTP-Versand schlicht falsch. Anlegen und erneutes Senden unterscheiden jetzt beide Fälle.
- **SMTP-Port 465 hätte nie verbunden**: implizites TLS braucht `secure: true`, nodemailers Standard ist `false` (STARTTLS-Aufwertung, korrekt für 587). Wird jetzt aus dem Port abgeleitet. Nicht durch einen Test abgedeckt — nur gegen ein echtes 465-Relay überprüfbar.
- **Der komplette Audit-Trail dieses Pakets war wirkungslos** (kritisch): alle zwölf `audit_events`-Inserts liefen über den **Nutzer-Client**. `authenticated` hat auf dieser Tabelle weder Insert-Grant (`202608020003_api_grants.sql` gewährt nur `select`) noch Insert-Policy — die Tabelle ist bewusst append-only („Inserts happen through privileged API procedures“, `202608020001:451`). Jeder Insert scheiterte an `permission denied for table audit_events`, wurde nur geloggt, und der Request antwortete trotzdem mit 2xx. Live gegen die lokale Datenbank bestätigt: `has_table_privilege('authenticated','public.audit_events','INSERT')` = `false`, null Insert-Policies. Nur `accept_invitation()` schrieb tatsächlich einen Eintrag, weil es `security definer` ist. Jetzt über den Service-Client, dasselbe Muster wie der Brand-Logo-Upload aus Paket 022. Abgesichert durch einen API-Test, der den geschriebenen Eintrag prüft (der Nutzer-Client-Fake wirft jetzt bei `audit_events`), plus zwei pgTAP-Assertions auf Grant und Policy-Abwesenheit.
- **`GET /v1/organizations/:id/invitations` verifizierte die Scope-Kette nicht** (dieselbe Klasse wie der `POST`-Fund oben): `departmentId`/`teamId` gingen ungeprüft in `requirePermission`, ein `department_admin` einer **fremden** Organisation kam damit durch die Berechtigungsprüfung für eine beliebige `organizationId`. Ein Leck entstand nur nicht, weil der zusammengesetzte Fremdschlüssel auf `invitations` eine solche Kombination ohnehin auf null Zeilen filtert — die Sicherheit hing also an einem Fremdschlüssel statt an einer Prüfung. Nutzt jetzt dasselbe `resolveInvitationScope()` wie `POST`, mit Regressionstest.
- **`authz.register_invitation_send` war für jeden angemeldeten Nutzer direkt aufrufbar** (Sicherheitsfund): das `authz`-Schema ist über PostgREST exponiert (`supabase/config.toml`, `schemas = [..., "authz"]` — die Oberfläche ruft dort `membership_scopes()` auf), die Funktion war an `authenticated` gegrantet und ist die einzige der neuen `authz`-Funktionen **ohne eigene Berechtigungsprüfung**. Damit konnte jeder authentifizierte Nutzer mit einer bekannten `organization_id` das Einladungskontingent einer beliebigen Adresse bis `send_count = 10` verbrauchen, ohne dass diese je eingeladen wurde — danach ist sie in diesem Verein **dauerhaft nicht mehr einladbar**, es gibt keinen Weg, den Zähler zurückzusetzen. Der Grant war überflüssig: beide Aufrufer (`create_invitation`, `resend_invitation`) sind `security definer`, keine RLS-Policy wertet die Funktion aus. Jetzt nur noch `service_role`, mit pgTAP-Assertions für beide Richtungen plus einem Test, dass der Einladungsflow weiter funktioniert.

- **`invitations` war für `authenticated` spaltenblind schreibbar**: `grant insert, update on table public.invitations` erlaubte jedem Inhaber von `member.invite`, `token_hash`, `email` oder `expires_at` einer fremden offenen Einladung direkt über PostgREST zu ändern — an der API und damit am Audit-Trail vorbei. Keine Rechteausweitung (`invitations_update` prüft weiter `can_assign_role`), aber unnötige Angriffsfläche; der Insert-Grant war zusätzlich toter Code, seit `POST /v1/invitations` über `create_invitation()` läuft. Beide RPCs gehören `postgres` (`bypassrls`) und brauchen deshalb überhaupt keinen Grant. Jetzt nur noch `grant update (revoked_at)` — das Widerrufen ist die einzige Schreibung über den Nutzer-Client. Die Policies bleiben bestehen: ohne Grant unerreichbar, aber sofort wieder wirksam, falls ein Grant zurückkehrt. Fünf pgTAP-Assertions auf die Spaltenrechte; dass `create_invitation`/`resend_invitation` weiter funktionieren, decken die bestehenden Tests ab, die beide als `authenticated` aufrufen.
- **`POST /v1/invitations/:id/revoke` widerrief auch eine bereits angenommene Einladung** (setzte `revoked_at` auf einer angenommenen Zeile). Ohne Wirkung auf die entstandene Mitgliedschaft, aber ein irreführender Audit-Eintrag; ein zweiter Widerruf erzeugte nur Rauschen. Jetzt dieselbe Bedingung wie `/resend`: nur eine offene Einladung ist widerrufbar, sonst 404.

Nicht behoben, bewusst offengelassen:

- **`POST /v1/memberships` legt für eine Team-Mitgliedschaft keine `viewer`-Zeile in der Abteilung an**, anders als `accept_invitation()`. Betroffene sehen deshalb keine Inhalte (jede Inhalts-Policy prüft `is_department_member`, siehe „Risiken“). Aus der Oberfläche nicht erreichbar (kein UI-Pfad zu dieser Route). **Bewusst nicht angeglichen**: die `viewer`-Zeile selbst entfällt in Paket 023 wieder — sie macht ein Teammitglied zum vollwertigen Abteilungsmitglied und zeigt ihm damit auch Entwürfe und Einreichungen der ganzen Abteilung, was der Nutzer ausdrücklich nicht will (ein Teammitglied soll sehen, was die Abteilung *veröffentlicht* hat, plus die Entwürfe des eigenen Teams). Siehe Paket 023, „Sichtbarkeit richtet sich nach dem Lebenszyklus“ — die Angleichung hier würde einen Zustand zementieren, der dort verschwindet. **✓ Erledigt**: `accept_invitation()` legt seit Paket 023 keine `viewer`-Zeile mehr an; dieser Punkt hat sich damit von selbst aufgelöst, wie hier vorhergesagt.
- **`GET /v1/organizations/:id/members` prüft keine Permission**, sondern verlässt sich allein auf RLS — bewusst, weil die Mitgliederliste vereinsweit sichtbar sein soll und es dafür keine passende Permission gibt. Folge: ein Nichtmitglied erhält `200 []` statt `403`, anders als bei jeder anderen Route.
- **Audit-Einträge sind nicht manipulationssicher, nur append-only**: `authenticated` hat kein `update`/`delete` auf `audit_events`, die Einträge sind aber unverkettet und unsigniert — wer Datenbankzugriff hat (Betreiber, Service-Role, ein Leak des Service-Keys), kann sie unbemerkt ändern oder löschen. Für „nachvollziehbar, wer wann welche Berechtigung vergeben hat“ reicht das heute; für kryptografische Nachweisbarkeit fehlt eine Hash-Kette (`prev_hash`/`hash` per Trigger) plus ein periodisch signierter Wurzelhash. Siehe Paket 020 (rechtliche Pflichten) als natürlichen Ort.
- **Wer einladen darf, ist heute nur über die Rollenvergabe steuerbar**, nicht über eine eigene Richtlinie: `member.invite` steckt fest in `department_admin` und `team_manager`. Der Verein bestimmt damit indirekt (durch die Rollenvergabe), welche Personen einladen dürfen, kann es aber einer Abteilung nicht rollenunabhängig entziehen. Das ist ein Vererbungsfall für `policy_settings`. **Entscheidung des Nutzers: in Paket 023 bauen** — als `invite_allowed boolean` (`null` = erben), dort im Datenmodell vermerkt. **✓ Erledigt**: `invite_allowed` sitzt an den Neuanlage-Stellen selbst (die drei `*_memberships_insert`-Policies, `invitations_insert`, `create_invitation()`), bewusst nicht in `has_department_permission`/`has_team_permission` — sonst wäre auch `change_membership_role()` (Rollenwechsel Bestehender) betroffen gewesen. Editierbar über `/struktur` (`PolicyFlagToggles.vue`), nicht über `/mitglieder`.
