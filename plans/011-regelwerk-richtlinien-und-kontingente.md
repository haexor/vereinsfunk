...
Umgesetzt in einer Migration (`2026080606_policies_and_review_routes.sql`), den Domain-Funktionen `resolveReviewRoute`/`evaluateSubmitPermission`/`evaluateQuota`/`resolveReviewers`/`resolveEffectiveConfig` in `packages/domain`, den entsprechenden Contracts- und API-Erweiterungen, sowie der Oberfläche auf `/einstellungen` (Richtlinienseite), `/mitglieder` (Vertrauen je Mitglied) und `/freigaben` (echte Stufen statt erfundener Beiträge). 235 pgTAP-Tests, 66 API-Tests, 35 Domain-Tests, `pnpm lint`/`typecheck`/`test`/`build` grün, manuell im Browser gegengeprüft (Richtlinie ändern und speichern mit echtem PUT-Roundtrip nach Reload, Prüfer hinzufügen/entfernen, Vertrauen-Block auf `/mitglieder`, ehrlicher Leerzustand auf `/freigaben`).
