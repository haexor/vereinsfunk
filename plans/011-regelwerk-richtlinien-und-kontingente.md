...
Umgesetzt in einer Migration (`2026080606_policies_and_review_routes.sql`), den Domain-Funktionen `resolveReviewRoute`/`evaluateSubmitPermission`/`evaluateQuota`/`resolveReviewers`/`resolveEffectiveConfig` in `packages/domain`, den entsprechenden Contracts- und API-Erweiterungen, sowie der Oberfläche auf `/einstellungen` (Richtlinienseite), `/mitglieder` (Vertrauen je Mitglied) und `/freigaben` (echte Stufen statt erfundener Beiträge). 222 pgTAP-Tests, 61 API-Tests, 35 Domain-Tests, `pnpm lint`/`typecheck`/`test`/`build` grün, manuell im Browser gegengeprüft (Richtlinie ändern und speichern mit echtem PUT-Roundtrip nach Reload, Prüfer hinzufügen/entfernen, Vertrauen-Block auf `/mitglieder`, ehrlicher Leerzustand auf `/freigaben`).
...
- `post_versions.effective_config_snapshot` bleibt **strukturell ungefüllt** — es gibt keine Stelle, die eine `post_versions`-Zeile überhaupt anlegt. Der Rückbau-Punkt aus `plans/README.md` bleibt bewusst offen, bis Paket 005/006 diesen Anlegepfad bauen.
- `/beitraege` und `/freigaben` zeigen ehrliche Leerzustände statt der vorherigen erfundenen Beiträge — nicht, weil sie fertig sind, sondern weil es noch keine echten Daten gibt, die sie zeigen könnten. Beide Dateien tragen einen Kommentar, der das für die nächste Session festhält.
