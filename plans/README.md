...
| 034 | [Ausgehende LLM-Provider-Aufrufe absichern und Kandidaten-Wiederherstellung nach Absturz](034-provider-aufrufe-absichern-und-generation-recovery.md) | 033, PR #50 | bereit; zwei beim Review von PR #50 gefundene, bewusst zurückgestellte Lücken — SSRF im Textgenerierungspfad (`packages/content-engine`s Adapter riefen `base_url` mit rohem `fetch` auf) und ein nach Worker-Absturz für immer auf `generating` hängender Kandidat, dessen Hatchet-Retry trotzdem Erfolg meldet |
