...
| 027 | [Fastify-API in fachliche Route-Module zerlegen](027-api-route-module-boundaries.md) | 026 | in Arbeit; PR #35 gemergt (Route-Kontext, Brand/Organisation, Struktur), PR #36 offen (Mitglieder/Einladungen, Richtlinien/Freigaben). `app.ts` von 8.301 auf 5.162 LoC reduziert. Extrahiert: Route-Kontext, Brand/Organisation, Struktur (Abteilungen/Teams), Mitglieder und Einladungen (`routes/members.ts`), Richtlinien und Freigaben (`routes/policies.ts`, inkl. Channel-Quotas). `routes/shared.ts` trägt inzwischen die komplette, domänenübergreifend gebrauchte Regelauflösung (`fetchPolicyRuleRows`/`computeRuleEntry`/`fetchMemberTrust` u. a. — auch von `POST /v1/submissions` benötigt) sowie `isAnyMemberOfOrganization`/`checkRateLimit`/`createAuditRecorder` (Audit-Logging als Factory, weil es `supabaseClients` aus der `buildApp`-Closure brauchte) statt je Modul dupliziert zu werden. Offen: Kanäle/OAuth/Publishing, Integration/Verzeichnis/Einwilligung, Datenschutz, Analytics, sowie eine noch zu klärende Einordnung von Plattform-Administration/LLM-Provider-Verwaltung (Paket 022) und `POST /v1/submissions` (Inhalts-Pipeline) — beide nicht Teil der ursprünglichen Domänenliste. Die im Plan vorgesehene Aufteilung von `app.test.ts` in `*.routes.test.ts` ist bislang für keine extrahierte Domäne erfolgt — alle 263 API-Tests laufen weiterhin zentral, siehe Plan „Current state“. |
