...
| 027 | [Fastify-API in fachliche Route-Module zerlegen](027-api-route-module-boundaries.md) | 026 | in Arbeit; `app.ts` von 8.301 auf 6.789 LoC reduziert. Extrahiert: Route-Kontext, Brand/Organisation, Struktur (Abteilungen/Teams), Mitglieder und Einladungen (`routes/members.ts`); dabei `routes/shared.ts` für die domänenübergreifend gebrauchten `fetchAllRows`/`resolveMembershipScope`/`toPermissionScope` eingeführt statt sie je Modul zu duplizieren. Offen: Richtlinien/Freigaben, Kanäle/OAuth/Publishing, Integration/Verzeichnis/Einwilligung, Datenschutz, Analytics, sowie eine noch zu klärende Einordnung der Plattform-Administration (Paket 022, nicht Teil der ursprünglichen Domänenliste). Die im Plan vorgesehene Aufteilung von `app.test.ts` in `*.routes.test.ts` ist bislang für keine extrahierte Domäne erfolgt — alle 263 API-Tests laufen weiterhin zentral, siehe Plan „Current state“. |
