...
| Nr. | Arbeitspaket | Abhängigkeiten | Status |
|---|---|---|---|
| 032 | [Mobile Textwerkstatt mit Stilprofilen sowie Foto- und Video-Anhang](032-mobile-textwerkstatt-mit-stilprofilen.md) | 002, 004, 025, 030 | bereit |
...
Sechster Befund, im CodeRabbit-Review zu PR #40 gefunden: `organization_consent_texts_immutable` in Paket 015 war ein `before update or delete`-Trigger. Obwohl `organization_consent_texts.organization_id` auf `organizations(id) on delete cascade` verweist, fing der Trigger auch die kaskadierte Löschung ab und verhinderte damit die vollständige Löschung eines Vereins mit vorhandenem Einwilligungstext. **✓ Behoben in Paket 015**: Der Trigger beschränkt sich auf `before update`; ein pgTAP-Regressionstest beweist die erfolgreiche FK-Kaskade.
