...
| 014 | [Integrationsrahmen und Mitgliederverzeichnis](014-integrationsrahmen-und-mitgliederverzeichnis.md) | 010 | erledigt; Produktivbetrieb ab 020 möglich; HTTP-Adapter und Foto-Upload fürs eigene Profil bewusst nicht enthalten; ein `apply`-Lauf ist weder transaktional noch gegen gleichzeitige/wiederholte Läufe gesichert (siehe ADR-009 „Bekannte Grenzen“, gehört zu 004) |
| 015 | [Einwilligungsverwaltung](015-einwilligungsverwaltung.md) | 002, 014 | erledigt; Produktivbetrieb ab 020 möglich; „Übernahme aus Quellsystem“ (`origin='imported'`) bewusst nicht gebaut (kein Zielsystem mit Testzugang, wie beim HTTP-Adapter aus 014); Textprüfung nur regelbasiert, kein Sprachmodell (Content-Pipeline 001–007 fehlt weiterhin); **offener Fund** (PR #40-Review zu Paket 032, 2026-08-11): `organization_consent_texts_immutable`-Trigger blockiert `ON DELETE CASCADE` beim Löschen einer Organisation, siehe Plan „Nachträglich gefundener Fehler, noch offen“ — nächster Schritt laut `plans/NEXT-SESSION.md` |
...
| 020 | [Rechtliche Pflichten und Datenschutzbetrieb](020-rechtliche-pflichten-und-datenschutzbetrieb.md) | 009, 012, 015 | erledigt; sechs Aufbewahrungsregeln (inkl. Einwilligungsnachweise), Betroffenenrechte, Verarbeitungsdokumentation, manipulationssicherer Audit-Trail mit echter Signaturprüfung, öffentliches Impressum je Verein; kein Hatchet-Cron (004 weiterhin „in Arbeit“), Läufe bis dahin manuell ausgelöst; vollständige Vereinskonto-Löschung bewusst nicht gebaut |
...
| Nr. | Arbeitspaket | Abhängigkeiten | Status |
|---|---|---|---|
| 032 | [Mobile Textwerkstatt mit Stilprofilen sowie Foto- und Video-Anhang](032-mobile-textwerkstatt-mit-stilprofilen.md) | 002, 004, 025, 030 | in Arbeit; Teilphase 1 (Verträge, tenant-sicheres Datenmodell, Provenienz, RLS-Tests, ADR-010) erledigt. Aktivierung von LLM, Editor und Anhängen ist korrekt durch die offenen Pläne 004 (Outbox/Worker) und 002 (privater Upload/Media Gate) gesperrt. |

Reihenfolge: Zuerst die noch offenen technischen Voraussetzungen 002 (private, freigabefähige Medien) und 004 (ID-only Outbox/Worker) vervollständigen. Danach 032 als zusammenhängenden vertikalen Slice umsetzen: Kompressionsspike vor dem Medien-Upload, Stilprofile und LLM-Evaluation vor dem mobilen Editor, Kandidaten vor akzeptierten Versionen, erst dann realer Freigabe-/Publishinganschluss. Die Stilvorgabe ist als sichtbares, attributbasiertes Profil konzipiert, nicht als Imitation einer realen Person.
