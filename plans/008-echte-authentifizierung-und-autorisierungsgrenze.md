...
- `supabase/migrations/202608020001_initial_tenant_foundation.sql:24-30`: `public.profiles` referenziert `auth.users`, aber es existiert **kein Trigger auf `auth.users`**, der beim Registrieren eine Profilzeile anlegt. `supabase/seed.sql:14-16` legt Profile manuell an. Ein echter Neuregistrierter hat kein Profil, und weil alle Memberships und `created_by`-Spalten auf `profiles(id)` verweisen, schlägt danach jede fachliche Operation fehl.
