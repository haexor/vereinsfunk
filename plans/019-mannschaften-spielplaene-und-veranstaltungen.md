...
- **Kein täglicher Job für Anlassvorschläge.** Paket 004 liefert nun die technische ID-only Outbox-/Worker-Ausführungsumgebung, aber keine fachliche Cron-Registrierung für diesen Anwendungsfall — wie bei `mark_stalled_approval_stages()`, `recompute_directory_minor_status()` und den beiden Funktionen aus 012. Die Vorschlagsliste ist zustandslos aus reinen Lesevergleichen berechenbar (Termine, vorhandene Beiträge, Kontingentstand, weggeklickte Zeitstempel) und wird deshalb bei jedem Aufruf des entsprechenden Endpunkts frisch berechnet — kein Job, kein gespeichertes Zwischenergebnis nötig. Eine spätere Cron-Anbindung kann denselben Endpunkt/dieselbe Funktion regelmäßig aufrufen.
