...
- **pgTAP-Test** `supabase/tests/brand_website_analysis.test.sql`: FORCE ROW LEVEL SECURITY, RPC-Verhalten (Job-Anlage, Abteilungs-Herleitung, Ablehnung eines zweiten gleichzeitigen Triggers, Überschreiben statt Historie bei einem erneuten Lauf), keine Direktschreibrechte für `authenticated`, Mandantentrennung.
