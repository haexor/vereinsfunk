...
describe('agent contracts', () => {
  ...
  it('accepts only the payload that matches the proposed tool', () => {
    ...
    expect(AgentActionProposalSchema.safeParse({ ...base, toolName: 'save_content_brief', input: { communicationGoal: 'inform', sourceMaterial: { facts: { Termin: 'Samstag' }, observations: [], quotes: [] }, systemStyleProfileSlug: 'klar_erklaerend', targetPlatforms: ['instagram'] } }).success).toBe(true)
    expect(AgentActionProposalSchema.safeParse({ ...base, toolName: 'start_text_generation', input: { communicationGoal: 'inform', sourceMaterial: { facts: { Termin: 'Samstag' }, observations: [], quotes: [] }, systemStyleProfileSlug: 'klar_erklaerend', targetPlatforms: ['instagram'] } }).success).toBe(true)
    expect(AgentActionProposalSchema.safeParse({ ...base, toolName: 'accept_text_candidate', input: { candidateId: '10000000-7000-4000-8000-000000000001' } }).success).toBe(true)
    expect(AgentActionProposalSchema.safeParse({ ...base, toolName: 'schedule_publication', input: { postVersionId: '10000000-5000-4000-8000-000000000001', platform: 'instagram', scheduledFor: null } }).success).toBe(true)
    expect(AgentActionProposalSchema.safeParse({ ...base, toolName: 'execute_publication', input: { publicationId: '10000000-8000-4000-8000-000000000001' } }).success).toBe(true)
    expect(AgentActionProposalSchema.safeParse({ ...base, status: 'confirmed', targetRefs: [{ entityType: 'text_workshop_drafts', id: '10000000-6000-4000-8000-000000000001' }], toolName: 'save_content_brief', input: { communicationGoal: 'inform', sourceMaterial: { facts: { Termin: 'Samstag' }, observations: [], quotes: [] }, systemStyleProfileSlug: 'klar_erklaerend', targetPlatforms: ['instagram'] } }).success).toBe(true)
